...
public class TeeUtility {
	...
	public static java.sql.Timestamp parseTimeStamp(String dateStr)
		...
		if (dateStr == null) {
			...
			utilDate = TeeUtility.parseDate(dateStr);
		}
		java.sql.Timestamp sqlDate = new java.sql.Timestamp(utilDate.getTime());
		return sqlDate;
	}
	...
	public static String parseDoubleStr(BigDecimal bg, int format) {
		return TeeUtility.getFormatedStr(bg.doubleValue(), format);
	}
	...
	public static String getRandomName() {
		String currTimeStr = TeeUtility
				.getCurDateTimeStr(TeeUtility.DATE_FORMAT_NOSPLIT);
	...
	public static String encodeURL(String srcStr) throws Exception {
		...
		return URLEncoder.encode(srcStr, TeeConst.DEFAULT_CODE);
	}
	...
	public static String decodeURL(String srcStr) throws Exception {
		...
		return URLDecoder.decode(srcStr, TeeConst.DEFAULT_CODE);
	}
	...
	/**
	  ...
	  public static String getDateFilter(String fieldName, String fieldValue, String opt) throws Exception {
	    if (TeeUtility.isNullorEmpty(fieldValue)) {
	      return "1=1";
	    }
	    StringBuffer rtBuf = new StringBuffer();    
	    String fullStr = TeeUtility.getDateTimeStr(TeeUtility.parseDate(fieldValue));
	    rtBuf.append(wrapDateField(fieldName, TeeSysProps.getString("db.jdbc.dbms")));
	    rtBuf.append(opt);
	    rtBuf.append("\'");
	 ...
	 /**
	  ...
	  public static String formatIdsQuote(String ids){
		 return  TeeStringUtil.formatIdsQuote(ids);
	  }
	  /**
	   * 将 List<TeePerson> 转成 map 存储
	   * @author zhp
	   * @createTime 2014-1-17
	   * @editTime 下午05:58:35
	   * @desc
	   */
	  public static Map personList2Map(List<TeePerson> persons){
			...
			for(int i=0;i<persons.size();i++){
				TeePerson p = (TeePerson)persons.get(i);
				ids = ids + p.getUuid() + ",";
	  ...
	  /**
	   * 比较一个字符串 是否在 另一个字符串里面 
	   * value 要比较的字符串
	   * values 以都好分隔的字符串  如：1,2,3,
	   * @author zhp
	  ...
	  public static boolean findIn(String value,String values){
		  ...
		  if(TeeUtility.isNullorEmpty(value) || TeeUtility.isNullorEmpty(values)){
			  return false;
		  }
		  values = TeeUtility.formatIdsQuote(values);
		  value = TeeUtility.formatIdsQuote(value);
	   ...
	   * @return
	   */
	  public static String fetchOriginChinese(String str){
		  TeeRegexpAnalyser regexp = new TeeRegexpAnalyser();
		  regexp.setText(str);
		  String tmp = regexp.replace(new String[]{"[^\\u4E00-\\u9FA5！\\^@#￥%……&\\*，；‘“？、。\\<\\>\\[\\]\\{\\}\\-=+\\|]"}, new TeeExpFetcher(){
	  ...
	  /**
	   * 获取人员 部门Sid 、 人员Sid  、角色Sid
	   * @author syl
	   * @date 2014-5-24
	   * @param person
	   * @return
	   */
	  public static int[] parsePersonDeptId_UserId_UserRoleId(TeePerson person){
		  int[] temp = new int[3];
		  int deptSid = 0;
		  int userSid = 0;
		  int userRoleSid = 0;
		  if(person != null){
			  if(person.getDept() != null){//部门
				  deptSid = person.getDept().getUuid();
			  }
			  userSid = person.getUuid();//人员
			  if(person.getUserRole() != null){//部门
				  userRoleSid = person.getUserRole().getUuid();
			  }
		  }
		  temp[0] = deptSid;
		  temp[1] = userSid;
	  ...
	  public static void main(String[] args) {
			String str = "英文的不要。！cat many";
			System.out.println(fetchOriginChinese(str));
		}
