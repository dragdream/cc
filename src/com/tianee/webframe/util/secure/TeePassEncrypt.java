...
public class TeePassEncrypt {
	...
	static public void main(String argv[]) throws NoSuchAlgorithmException {
		...
		/*String encryptPass = crypt("111");
		TeePassEncrypt te = new TeePassEncrypt();
		*/
	
		System.out.println(checkCrypt("1","$1$roT.ukLM$zh.GVi0TQ/1dRZ/FupaKL/"));
	...
	static public final String crypt(String password)
			throws NoSuchAlgorithmException {
		StringBuffer salt = new StringBuffer();//加盐 
		java.util.Random randgen = new java.util.Random();//随机取8个字符串
		while (salt.length() < 8) {
			int index = (int) (randgen.nextFloat() * SALTCHARS.length());
			salt.append(SALTCHARS.substring(index, index + 1));
		}
		return TeePassEncrypt.crypt(password, salt.toString());
	}
	...
	static public final String crypt(String password, String salt)
		...
		if (salt.indexOf('$') != -1) {//如存在$
			salt = salt.substring(0, salt.indexOf('$'));//从截取从第一位到 第一次出现$的位置，不去$
		}
