package com.tianee.webframe.data;
...
public class TeeDataRecord {
	 //除编码、名称和父级编码以外的字段值
	   ...
	   * 连接另外一个记录集
	   */
	  public void join(TeeDataRecord other) {
	    if (other == null) {
	      return;
	    }
	    ...
	    for (int i = 0; i < fieldCnt; i++) {
	      ...
	      if (fieldValue == null) {
	        ...
	        if (Integer.class.equals(fieldType)) {        
	          ...
	          value = TeeUtility.getFormatedStr(((Double)fieldValue).doubleValue(), TeeUtility.WITHOUTGROUP);
	        }else if (Date.class.equals(fieldType)) {
	          value = "\"" + TeeUtility.getDateTimeStr((Date)fieldValue) + "\"";
	        }else {
	          if (fieldValue == null) {
	            value = "\"\"";
	          }else {
	            String tmpStr = TeeUtility.null2Empty(fieldValue.toString());
	  ...
	  public Object clone(String fieldNamePrefix) {
	    TeeDataRecord rtObj = new TeeDataRecord();
	    
	    for (int i = 0; i < fieldCnt; i++) {
	      String fieldName = getNameByIndex(i);
	      Object fieldValue = getValueByIndex(i);
	      if (!TeeUtility.isNullorEmpty(fieldNamePrefix)) {
	  ...
	  public String toString() {
	    return "TeeDataRecord [fieldCnt=" + fieldCnt + ", fieldIndexMap="
