...
public class TeeXDocNumCtrl extends TeeCtrl{
	...
	public String getCtrlHtml4Process(TeeFormItem formItem,
		...
		if(auto && "".equals(data)){
			...
			tag.getAttributes().put("value", data);
		}
		
		if(!writable){
			tag.getAttributes().put("disabled", "disabled");
		}else{
			tag.getAttributes().put("onclick", "docNumOpening(this)");
			tag.getAttributes().put("readonly", "");
			tag.getAttributes().put("writable", "");
		}
		tag.getAttributes().put("required", String.valueOf(required));
		
		
		return wrap(writable,tag.toString(),tag.getAttributes().get("value"),attrs.get("style"),"",formItem.getItemId(),formItem.getTitle());
	}
	...
	public String getCtrlHtml4MobileProcess(TeeFormItem formItem,
		...
		return wrap(writable,tag.toString(),tag.getAttributes().get("value"),"","",formItem.getItemId(),formItem.getTitle());
	}
