...
public class BudgetDeptAuditPlugin extends TeeWfPlugin{
	...
	public TeeJsonProxy beforeTurnnext() {
		...
		//获取数据库方言
				...
				try{
					...
					//获取部门显示值  
					String deptId = this.flowRunDatas.get("EXTRA_"+formItemIdentities.get("公司部门").split("_")[1]);
					...
					if(TeeUtility.isNullorEmpty(deptId)){
                    	...
                    	teeJsonProxy.setRtMsg("还未选择所属公司部门！");
						...
						String sql = "select BD.UUID from BUDGET_DEPT BD where BD.DEPT_ID=? and BD.YEAR=?";
						List<TeeDeptBudget> list = dbUtils.queryToArrayList(sql, new Object[]{deptId,year});
						if(list.size()<11){
							for (int i = 0; i < str.length; i++) {
								...
								dbUtils.executeInsert("insert into budget_dept("
										+"uuid,"
										+"amount,"
										+"month,"
										+"year,"
							...
							TeeDbUtility.rollback(dbConn);
							teeJsonProxy.setRtMsg("该公司部门年度预算已存在！");
							teeJsonProxy.setRtState(false);
						}
					}
	...
	public TeeJsonProxy beforeSave() {
		// TODO Auto-generated method stub
		/*	TeeJsonProxy teeJsonProxy=new TeeJsonProxy();
		String str=this.flowRunDatas.get(formItemIdentities.get("备注"));
		if("".equals(str)){
			teeJsonProxy.setRtMsg("请填写备注");
			teeJsonProxy.setRtState(false);
		}else{
		teeJsonProxy.setRtState(true);
