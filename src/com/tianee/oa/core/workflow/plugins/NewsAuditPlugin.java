...
public class NewsAuditPlugin extends TeeWfPlugin{
...
TeeJsonProxy jsonProxy = new TeeJsonProxy();
		...
		try{
			...
			//获取新闻类型
			String typeId = this.flowRunDatas.get(formItemIdentities.get("新闻类型选择"));
			...
			//新闻发布时间
			String newsTime =this.flowRunDatas.get(formItemIdentities.get("新闻发布时间"));
			if(newsTime==null||newsTime.equals("")){
				Date date=new Date();
				DateFormat f=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				newsTime=f.format(date);
			}
			//有效期结束
			//String endTime = this.flowRunDatas.get(formItemIdentities.get("有效期结束"));
			
			//是否显示为重要  0和1
			String isTop = this.flowRunDatas.get(formItemIdentities.get("重要新闻"));
			
			//公告内容
			String content = this.flowRunDatas.get(formItemIdentities.get("新闻内容"));
			...
			Object newsId = dbUtils.executeInsert("insert into news("
					...
					+ "values(?,?,?,?,?,?,?,?,0)", dialect, 
					new Object[]{
							TeeDateUtil.parseDate(newsTime),//新闻发布时间    日期类型	
							content,//公告内容  字符串型
			...
			if("0".equals(isAllPriv)){
				...
				if(!TeeUtility.isNullorEmpty(roleIds)){
					...
					for(int i=0;i<roleIdsArray.length;i++){
						dbUtils.executeInsert("insert into news_user_role_priv("
								+ "news_id,"
								+ "role_uuid) "
								+ "values(?,?)", dialect, 
								new Object[]{
			...
			smsData.put("moduleNo", "020");//020为新闻管理，具体模块代号可以参考　module_sort表
			//URL链接地址
			smsData.put("remindUrl", "/system/core/base/news/person/readNews.jsp?id="+newsId+"&isLooked=0&sid="+newsId);
			//将消息模型加入消息列表中
