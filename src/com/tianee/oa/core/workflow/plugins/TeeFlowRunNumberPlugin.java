...
public class TeeFlowRunNumberPlugin extends TeeWfPlugin {
	...
	public void afterTurnnext() {
		...
		if("分管领导意见".equals(prcsName) && "部门负责人".equals(person.getUserRole().getRoleName())){
			...
			if(runPrcs.getFlag()==4){//流程结束
				object.put("flag", "2");//0提交 1审批通过 2：流程结
			}else{
				object.put("flag", "1");//0提交 1审批通过 2：流程结
			}
			...
			Map m = teeFlowRunCtrlFeedBackDao.executeNativeUnique("select f.END_TIME as endTime from FLOW_PROCESS p,FLOW_RUN_PRCS f where p.SID=f.FLOW_PRCS and f.FLOW_ID=1032 and p.PRCS_ID=7 and f.RUN_ID=?", new Object[]{runId});
				...
				if(!"".equals(result) && ("分管领导意见".equals(prcsName) || "总队长意见".equals(prcsName))){
		        	JSONObject object2 = JSONObject.fromObject(result);
		        	   ...
		        	   if(b){
		        		   ...
		        		   //DATA_35 实际开始实际  DATA_56 实际开时间 DATA_55 实际结束时间 DATA_57 实际结时间
		        		   int executeNativeUpdate = teeFlowRunCtrlFeedBackDao.executeNativeUpdate("update tee_f_r_d_1032 set DATA_54=?,DATA_35=?,DATA_55=?,DATA_56=?,DATA_57=? where RUN_ID=?", new Object[]{workDate,beginTime,endTime,bTime,eTime,runId});
