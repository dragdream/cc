 ...
 * 
 */
public class TeeMimeMessageUtil {
  /**
   * 判断邮件中是否含有附件
   * 
   * @param msg
      ...
      if (part.isMimeType("text/plain") || part.isMimeType("text/html")) {
        ...
        if (part.isMimeType("multipart/alternative")) {
          ...
          for (int k = 1; count > 1 && k < count; k++) {
            ...
            try {
              File attFile = new File(TeeSysProps.getTmpPath(), userName
                  .concat(attname));
            ...
            if (Content_ID != null && Content_ID.length > 0) {
              String cid_name = Content_ID[0].replaceAll("<", "").replaceAll(
                  ">", "");
              body = body
                  .replaceAll("cid:" + cid_name, TeeSysProps.getTmpPath()
                      .concat("/").concat(userName.concat(attname)));
            }
          }
          sb.append(body);
          return sb.toString();
  ...
  public static String getFrom(Message mimeMessage){
	  ...
	  try{
	    ...
	    personal = MimeUtility.decodeText(personal);
	     ...
	     try {      
              if (personal.equals(new String(personal.getBytes(encode), encode))) {      
  ...
  public static String getSubject(Message mimeMessage)
    ...
    try {
      ...
      subject = MimeUtility.decodeText(TeeStringUtil.getString(subject));
...
//        subject = "";
   ...
   * 保存附件
   */
  public static List<TeeAttachment> saveAttachMent(Part part, TeeBaseUpload baseUpload,TeePerson person)
      throws Exception {
    String fileName = "";
...
//                  fileName = getFromBASE64(fileName.substring("=?gb18030?b?".length(),fileName.indexOf("?=")),"gb18030");
//                }
//              
//              if (fileName.toLowerCase().startsWith("=?gbk?q?") && fileName.endsWith("?=")) {
//            	  fileName = QuotedPrintable.decode(fileName.getBytes(), "GBK").replace("=?gbk?q?", "").replace("=?GBK?Q?", "").replace("?=", "");
//                }
//                if (fileName.toLowerCase().startsWith("=?gb2312?q?") && fileName.endsWith("?=")) {
//                  fileName = getFromBASE64(fileName.substring("=?gb2312?q?".length(),fileName.indexOf("?=")),"gbk");
//                }
//                if (fileName.toLowerCase().startsWith("=?utf-8?q?") && fileName.endsWith("?=")) {
//                  fileName = getFromBASE64(fileName.substring("=?utf-8?q?".length(),fileName.indexOf("?=")),"utf-8");
//                }
//                if (fileName.toLowerCase().startsWith("=?gb18030?q?") && fileName.endsWith("?=")) {
//                    fileName = getFromBASE64(fileName.substring("=?gb18030?q?".length(),fileName.indexOf("?=")),"gb18030");
//                  }
//              
...
//                fileName = MimeUtility.decodeText(fileName);
//              }
        	  fileName = MimeUtility.decodeText(TeeStringUtil.getString(fileName));
              
              attachList.add(saveFile(fileName, mpart.getInputStream(),
            		  baseUpload,person));
          }
          
        } else if (mpart.isMimeType("multipart/*")) {
        	attachList.addAll(saveAttachMent(mpart, baseUpload,person));
        } else {
          fileName = mpart.getFileName();
//          if ((fileName != null)
...
//          }
    	...
    	attachList.addAll(saveAttachMent((Part) part.getContent(), baseUpload,person));
    }
    return attachList;
  }
...
//          }
   ...
   * @throws Exception
   */
  public static TeeAttachment saveFile(String fileName, InputStream in,
      TeeBaseUpload baseUpload,TeePerson person) throws Exception {
    return baseUpload.singleAttachUpload(in, in.available(), fileName, null, TeeAttachmentModelKeys.EMAIL, person);
  }
