...
public class TeeBlowfish {
	...
	public byte[] encrypt(final byte[] source, final int offset,
		...
		final byte[] output = new byte[4 + (blocks * BLOCK_SIZE)];
	...
	public byte[] decryptBytes(final byte[] source) {
		encrypting = false;
		final int length = ((source[0]&0x000000ff)<<24)|((source[1]&0x000000ff)<<16)|((source[2]&0x000000ff)<<8)|(source[3]&0x000000ff);//((source[2] & 0xFF) * 256) + (source[1] & 0xFF);
		final int blocks = (length / BLOCK_SIZE)
				+ (length % BLOCK_SIZE != 0 ? 1 : 0);
...
/*
	public static void main(String args[]) throws Exception {
		...
		// 要加密的字符串
		String str = "http://www.open-open.com";

		TeeBlowfish bf1 = new TeeBlowfish();

		bf1.init(true, keyByte);
		// 加密后的数组
		byte[] tt = bf1.encrypt(str);
		// 解密
		TeeBlowfish bf2 = new TeeBlowfish();
		bf2.init(true, keyByte);
		System.out.println(bf2.decryptString(tt));
	}*/
