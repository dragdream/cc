package com.tianee.thirdparty.wenshu.plugins;

import java.util.Map;

import com.jacob.req.JacobRequest;
import com.jacob.req.JacobResponse;
import com.tianee.webframe.util.str.TeeUtility;

public class CRKDPlugins extends TeeWenShuPlugin{

	@Override
	public String process(JacobRequest jacobRequest, String wordId,
			Map<String, String> data) {
		/*	String base64="";
		data.put("单名称","送达单位");
		data.put("送交部门","xx部");
		data.put("当事人名称","xxx");
		data.put("文号编号","123");
		data.put("单名称1","送达单位");
		data.put("编号","002");
		data.put("图书成品册","234");
		data.put("图书散文令","erwrdx");
		data.put("报纸成品份","23");
		data.put("图书散文令","133");
		data.put("期刊成品本","123");
		data.put("期刊散文本","123");
		data.put("光盘成品张","123");
		data.put("光盘散文张","123");
		data.put("录音录像带成品盒","123");
		data.put("录音录像带散盒","123");
		data.put("其他物品","123");
		data.put("送交主办人签名",base64);
		data.put("送交协办人签名",base64);
		data.put("接收协办人签名",base64);
		data.put("接收主办人签名",base64);
		data.put("交送人签名日期","2018-12-01");
		data.put("接收人签名日期","2018-12-01");
		data.put("页眉","编号：21424324324234");*/
		
		
		
		
		//清除图片签章
		String sjzbr = data.get("送交主办人签名");
		data.remove("送交主办人签名");
		String sjxbr = data.get("送交协办人签名");
		data.remove("送交协办人签名");
		String jsxbr = data.get("接收协办人签名");
		data.remove("接收协办人签名");
		String jszbr = data.get("接收主办人签名");
		data.remove("接收主办人签名");
		
		/*String bz = data.get("备注");
		String bz1 = data.get("备注1");
		String bz2 = data.get("备注2");
		String bz3 = data.get("备注3");
		String bz4 = data.get("备注4");
		String bz5 = data.get("备注5");
		String bz6 = data.get("备注6");*/
		
		
		//套入书签
		JacobResponse jacobResponse = jacobRequest.replaceBookmarks(wordId, data);
		String newWordId = jacobResponse.fileId;//获取替换书签后的word文件id
		
		//判断签名是否存在
		String scriptRun ="";
		if(!TeeUtility.isNullorEmpty(sjzbr)){
			scriptRun += "jacob.setBookMarksPicture(\"送交主办人签名\",\""+sjzbr+"\",6,-1,30);";
		}
		String scriptRun1 ="";
		if(!TeeUtility.isNullorEmpty(sjxbr)){
			scriptRun += "jacob.setBookMarksPicture(\"送交协办人签名\",\""+sjxbr+"\",6,-1,30);";
		}
		String scriptRun2 ="";
		if(!TeeUtility.isNullorEmpty(jszbr)){
			scriptRun += "jacob.setBookMarksPicture(\"接收主办人签名\",\""+jszbr+"\",6,-1,30);";
		}
		String scriptRun3 ="";
		if(!TeeUtility.isNullorEmpty(jsxbr)){
			scriptRun += "jacob.setBookMarksPicture(\"接收协办人签名\",\""+jsxbr+"\",6,-1,30);";
		}
		
		
		/*if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 9, 2);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 2, 6);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 3, 6);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 4, 6);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 6, 6);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 7, 6);";  

		}
		if(TeeUtility.isNullorEmpty(bz)){
			scriptRun = "jacob.setTableCellBorderXieXian(1, 8, 6);";  

		}*/
		/*scriptRun = "jacob.setTableCellBorderXieXian(1, 1, 3);";
		scriptRun = "jacob.setTableCellBorderXieXian(1, 5, 3);"; */
	  jacobResponse = jacobRequest.scriptRun(scriptRun, newWordId);
	  newWordId = jacobResponse.fileId;
	  jacobResponse = jacobRequest.scriptRun(scriptRun1, newWordId);
	  newWordId = jacobResponse.fileId;
	  jacobResponse = jacobRequest.scriptRun(scriptRun2, newWordId);
	  newWordId = jacobResponse.fileId;
	  jacobResponse = jacobRequest.scriptRun(scriptRun3, newWordId);
	  newWordId = jacobResponse.fileId;
	  
		
		return newWordId;
	}

}
