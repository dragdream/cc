package com.tianee.thirdparty.wenshu.plugins;

import java.util.List;
import java.util.Map;

import com.jacob.req.JacobRequest;
import com.jacob.req.JacobResponse;
import com.tianee.webframe.util.str.TeeJsonUtil;
import com.tianee.webframe.util.str.TeeUtility;

//物品清单
public class WPQDPlugin extends TeeWenShuPlugin{

	@Override
	public String process(JacobRequest jacobRequest, String wordId,
			Map<String, String> data) {
		
		/*data.put("文号文", "顺");
		data.put("文号年", "2017");
		data.put("文号", "20007");
		data.put("单位合计", "令");
		data.put("数量合计", "5");
		data.put("备注合计", "散页");
		data.put("现场负责人或受托人意见", "核对无误");
		data.put("主办人签名", "");
		data.put("主办人执法证号", "02120016");
		data.put("协办人签名", "");
		data.put("协办人执法证号", "02120018");
		data.put("当事人盖章日期", "2017年8月2日");
		data.put("行政机关盖章日期", "2017年8月2日");
		data.put("行政机关落款", "北京市顺义区文化委员会");
		data.put("页眉", "编号：89757");
		
		String info="[{\"序号\":\"1\",\"物品名称\":\"香蕉\",\"计量单位\":\"个\",\"数量\":\"5\",\"备注\":\"111\"},"
				+ "   {\"序号\":\"2\",\"物品名称\":\"苹果\",\"计量单位\":\"颗\",\"数量\":\"6\",\"备注\":\"222\"}]";
		data.put("物品列表信息",info);*/
		
		
		
		//处理物品列表信息
		String jsonStr=data.get("物品列表信息");
		data.remove("物品列表信息");

		//base64图片单独处理
		String zbrqm= data.get("主办人签名");
		data.remove("主办人签名");
		
		String xbrqm= data.get("协办人签名");
		data.remove("协办人签名");
		
		String dsrqz = data.get("当事人签名");
		data.remove("当事人签名");
		
		String dsryj = data.get("当事人意见");
		data.remove("当事人意见");
		
		
		//套入书签
		JacobResponse response = jacobRequest.replaceBookmarks(wordId, data);
		String newWordId = response.fileId;//获取套完书签后的wordId
		
		//判断签名是否存在
		String scriptRun = "";
		if(!TeeUtility.isNullorEmpty(zbrqm)){
			scriptRun = "jacob.setBookMarksPicture(\"主办人签名\",\""+zbrqm+"\",6,-1,30);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(xbrqm)){
			scriptRun = "jacob.setBookMarksPicture(\"协办人签名\",\""+xbrqm+"\",6,-1,30);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(dsrqz)){
			scriptRun += "jacob.setBookMarksPicture(\"当事人签名\",\""+dsrqz+"\",6,-1,20);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(dsryj)){
			scriptRun += "jacob.setBookMarksPicture(\"当事人意见\",\""+dsryj+"\",6,-1,20);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		String scriptRun1 = "";
		//处理物品列表
		if(!TeeUtility.isNullorEmpty(jsonStr)){
			List<Map<String, String>> mapList=TeeJsonUtil.JsonStr2MapList(jsonStr);
			if(mapList!=null&&mapList.size()>0){
				for(int i=0;i<mapList.size();i++){
					scriptRun1 = " jacob.putTxtToCell(1,\""+(i+2)+"\",1,\""+mapList.get(i).get("序号")+"\");"; 
					scriptRun1 += " jacob.putTxtToCell(1,\""+(i+2)+"\",2,\""+mapList.get(i).get("物品名称")+"\");";
					scriptRun1 += " jacob.putTxtToCell(1,\""+(i+2)+"\",3,\""+mapList.get(i).get("计量单位")+"\");"; 
					scriptRun1 += " jacob.putTxtToCell(1,\""+(i+2)+"\",4,\""+mapList.get(i).get("数量")+"\");"; 
					scriptRun1 += " jacob.putTxtToCell(1,\""+(i+2)+"\",5,\""+mapList.get(i).get("备注")+"\");"; 
					response = jacobRequest.scriptRun(scriptRun1, newWordId);
					newWordId = response.fileId;
				}
			}
			
		}
		
		return newWordId;
	}

}
