 ...
 * 案件集体讨论记录 (29)
 * */
public class AJJTTLJLPlugin extends TeeWenShuPlugin{
...
//		//案件名称
//		data.put("案件名称", "关于XXX案件");
//		//讨论开始时间
//		data.put("讨论开始时间", "2018年10月16日 08时30分");
//		//讨论结束时间
//		data.put("讨论结束时间", "12时10分");
//		//讨论地点
//		data.put("讨论地点", "北京海淀区");
//		//主持人
//		data.put("主持人", "小米");
...
//		data.put("案由", "关于XXX案由");
//		//案情及处罚
//		data.put("案情及处罚", "XXX处罚");
//		//审核意见
//		data.put("审核意见", "同意");
//		//讨论记录
//		data.put("讨论记录", "关于XXX讨论记录");
//		//讨论结论性解决
//		data.put("讨论结论性意见", "XXX");
//		//参加讨论人员签名
//		data.put("参加讨论人员签名", img);
		
		//讨论人员签名1
		String tlryqm1 = data.get("讨论人员签名1");
		data.remove("讨论人员签名1");
		
		//讨论人员签名2
		String tlryqm2 = data.get("讨论人员签名2");
		data.remove("讨论人员签名2");
		
		
		//讨论人员签名3
		String tlryqm3 = data.get("讨论人员签名3");
		data.remove("讨论人员签名3");
		
		
		//讨论人员签名4
		String tlryqm4 = data.get("讨论人员签名4");
		data.remove("讨论人员签名4");
		
		
		//讨论人员签名5
		String tlryqm5 = data.get("讨论人员签名5");
		data.remove("讨论人员签名5");
		
		
		//讨论人员签名6
		String tlryqm6 = data.get("讨论人员签名6");
		data.remove("讨论人员签名6");
		...
		//判断签名是否存在
		if(!TeeUtility.isNullorEmpty(tlryqm1)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名1\",\""+tlryqm1+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		
		if(!TeeUtility.isNullorEmpty(tlryqm2)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名2\",\""+tlryqm2+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		
		if(!TeeUtility.isNullorEmpty(tlryqm3)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名3\",\""+tlryqm3+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(tlryqm4)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名4\",\""+tlryqm4+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		
		if(!TeeUtility.isNullorEmpty(tlryqm5)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名5\",\""+tlryqm5+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(tlryqm6)){
			String scriptRun = "jacob.setBookMarksPicture(\"讨论人员签名6\",\""+tlryqm6+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
