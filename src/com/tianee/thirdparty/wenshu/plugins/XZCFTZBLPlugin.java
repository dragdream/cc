package com.tianee.thirdparty.wenshu.plugins;

import java.util.Map;

import com.jacob.req.JacobRequest;
import com.jacob.req.JacobResponse;
import com.tianee.webframe.util.str.TeeUtility;
/**
 * 行政处罚听证笔录(22)
 * */
public class XZCFTZBLPlugin extends TeeWenShuPlugin{

	@Override
	public String process(JacobRequest jacobRequest, String wordId,
			Map<String, String> data) {
//		String img="";
//		data.put("页眉", "编码：222222222");
//		//案由
//		data.put("案由", "XXX案由");
//		//听证开始时间
//		data.put("听证开始时间", "2018年10月16日 08时20分");
//		//听证结束时间
//		data.put("听证结束时间", "14时20分");
//		//听证地点
//		data.put("听证地点", "北京海淀区");
//		//听证主持人
//		data.put("听证主持人", "王小明");
//		//记录人
//		data.put("记录人", "李明");
//		//主办人
//		data.put("主办人", "小红");
//		//主办人执法证号
//		data.put("主办人执法证号", "100010");
//		//协办人
//		data.put("协办人", "小兰");
//		//协办人执法证号
//		data.put("协办人执法证号", "100011");
//		//当事人名称
//		data.put("当事人名称", "李四");
//		//当事人类型
//		data.put("当事人类型", "职工");
//		//性别
//		data.put("性别", "男");
//		//联系电话
//		data.put("联系电话", "44444444444");
//		//住所
//		data.put("住所", "北京海淀区");
//		//委托代理人1
//		data.put("委托代理人1", "王三");
//		//性别1
//		data.put("性别1", "男");
//		//联系电话1
//		data.put("联系电话1", "3333333333");
//		//委托代理人2
//		data.put("委托代理人2", "王二");
//		//性别2
//		data.put("性别2", "男");
//		//联系电话2
//		data.put("联系电话2", "66666666666");
//		//证人
//		data.put("证人", "丽丽");
//		//其他人员
//		data.put("其他人员", "红红、毛毛");
//		//听证笔录
//		data.put("听证笔录", "关于XXX笔录");
//		//当事人签名
//		data.put("当事人签名", img);
//		//委托代理人签名
//		data.put("委托代理人签名", img);
//		//承办人签名
//		data.put("承办人签名", img);
//		//听证主持人签名
//		data.put("听证主持人签名", img);
//		//听证记录人签名
//		data.put("听证记录人签名", img);
		//当事人签名
		String dsrqm = data.get("当事人签名");
		data.remove("当事人签名");
		//委托代理人签名
		String wtdlrqm = data.get("委托代理人签名");
		data.remove("委托代理人签名");
		//承办人签名
		String cbrqm1 = data.get("承办人签名1");
		data.remove("承办人签名1");
		
		String cbrqm2 = data.get("承办人签名2");
		data.remove("承办人签名2");
		//听证主持人签名
		String tzzcrqm = data.get("听证主持人签名");
		data.remove("听证主持人签名");
		//听证记录人签名
		String tzjlrqm = data.get("听证记录人签名");
		data.remove("听证记录人签名");
				
		
		//套入书签
		JacobResponse response = jacobRequest.replaceBookmarks(wordId, data);
		String newWordId = response.fileId;//获取套完书签后的wordId
		
		//判断签名是否存在
		if(!TeeUtility.isNullorEmpty(dsrqm)){
			String scriptRun = "jacob.setBookMarksPicture(\"当事人签名\",\""+dsrqm+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(wtdlrqm)){
			String scriptRun = "jacob.setBookMarksPicture(\"委托代理人签名\",\""+wtdlrqm+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(cbrqm1)){
			String scriptRun = "jacob.setBookMarksPicture(\"承办人签名1\",\""+cbrqm1+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(cbrqm2)){
			String scriptRun = "jacob.setBookMarksPicture(\"承办人签名2\",\""+cbrqm2+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(tzzcrqm)){
			String scriptRun = "jacob.setBookMarksPicture(\"听证主持人签名\",\""+tzzcrqm+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(tzjlrqm)){
			String scriptRun = "jacob.setBookMarksPicture(\"听证记录人签名\",\""+tzjlrqm+"\",6,-1,30)";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
	
		return newWordId;
	}

}
