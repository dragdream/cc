package com.tianee.thirdparty.wenshu.plugins;

import java.util.Map;

import com.jacob.req.JacobRequest;
import com.jacob.req.JacobResponse;
import com.tianee.webframe.util.str.TeeUtility;

public class SQWTSPlugins extends TeeWenShuPlugin{

	@Override
	public String process(JacobRequest jacobRequest, String wordId,
			Map<String, String> data) {
		String base64="";
		/*data.put("委托人","送达单位");
		data.put("法定代表人","xx文");
		data.put("身份证号","xx");
		data.put("受委托人1","123");
		data.put("工作单位1","xxxx人民法院");
		data.put("职务1","决定xxxxxxxxx");
		data.put("身份证号1","2018-10-15");
		data.put("受委托人2",base64);
		data.put("工作单位2","xxxxxxx");
		data.put("职务2","2018-10-15");
		data.put("身份证号2","123231312");
		data.put("代理人","123231312");
		data.put("配合","123231312");
		data.put("事件","123231312");
		data.put("委托人签字",base64);
		data.put("法定代表人签字",base64);
		data.put("签发日期","123231312");
		data.put("页眉","编号：21424324324234");*/
		
		
		//清除图片签章
		/*String wtr = data.get("委托人签字");
		data.remove("委托人签字");
		String fddbr = data.get("法定代表人签字");
		data.remove("法定代表人签字");*/
		
		
		//套入书签
		JacobResponse jacobResponse = jacobRequest.replaceBookmarks(wordId, data);
		String newWordId = jacobResponse.fileId;//获取替换书签后的word文件id
		
		/*//判断签名是否存在
		String scriptRun ="";
		if(!TeeUtility.isNullorEmpty(wtr)){
			scriptRun += "jacob.setBookMarksPicture(\"委托人签字\",\""+wtr+"\",6,-1,30);";
		}
		String scriptRun1 ="";
		if(!TeeUtility.isNullorEmpty(fddbr)){
			scriptRun1 += "jacob.setBookMarksPicture(\"法定代表人签字\",\""+fddbr+"\",6,-1,30);";
		}
		
	 jacobResponse = jacobRequest.scriptRun(scriptRun, newWordId);
	 newWordId = jacobResponse.fileId;
	 jacobResponse = jacobRequest.scriptRun(scriptRun1, newWordId);
	 newWordId = jacobResponse.fileId;*/
		return newWordId;
	}

}
