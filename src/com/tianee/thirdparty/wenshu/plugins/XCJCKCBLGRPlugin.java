package com.tianee.thirdparty.wenshu.plugins;

import java.util.Map;

import com.jacob.req.JacobRequest;
import com.jacob.req.JacobResponse;
import com.tianee.webframe.util.str.TeeUtility;
//现场检查（勘察）笔录（个人用）
public class XCJCKCBLGRPlugin extends TeeWenShuPlugin{

	@Override
	public String process(JacobRequest jacobRequest, String wordId,
			Map<String, String> data) {
		
		/*data.put("页眉", "编号：2323232323");
		
		data.put("文号文", "密");
		data.put("文号年", "2017");
		data.put("文号", "000045");
		data.put("当事人姓名", "刘先树");
		data.put("字号名称", "北京刘先树运输户");
		data.put("性别", "男");
		data.put("联系电话", "15910424278");
		data.put("当事人住址", "山东省夏津县田庄乡郑庄村207号");
		data.put("证件名称", "中华人名共和国居民身份证");
		data.put("证件号码", "371427198910024355");
		data.put("现场负责人", "刘先树");
		data.put("职务", "经营者职务无");
		data.put("检查（勘验）地点", "北京市密云区密云路南侧");
		data.put("检查（勘验）开始时间", "2017年8月4日9时55分");
		data.put("检查（勘验）结束时间", "2017年8月4日10时22分");
		data.put("现场检查情况", "北京市密云区文化委员会行政执法人员xxxx");
		data.put("执法人员1签名", "");
		data.put("执法人员2签名", "");
		data.put("执法证号1", "02150014");
		data.put("执法证号2", "02150011");
		data.put("当事人职务", "经营者");
		data.put("行政机关落款", "北京市密云区文化委员会");
		data.put("当事人签字日期", "2017年8月4日");
		data.put("行政机关盖章日期", "2017年8月4日");*/
		
		
		//base64图片单独处理
		String zfryqm1= data.get("执法人员1签名");
		data.remove("执法人员1签名");
		
		String zfryqm2= data.get("执法人员2签名");
		data.remove("执法人员2签名");
		
		String gz = data.get("公章测试");
		data.remove("公章测试");
		
		String xcfzryj = data.get("现场负责人意见");
		data.remove("现场负责人意见");
		
		String xcfzrqm = data.get("现场负责人签名");
		data.remove("现场负责人签名");
		
		String dsrqz = data.get("当事人签名");
		data.remove("当事人签名");
		
		String dsryj = data.get("当事人意见");
		data.remove("当事人意见");
		
		//套入书签
		JacobResponse response = jacobRequest.replaceBookmarks(wordId, data);
		String newWordId = response.fileId;//获取套完书签后的wordId
		
		//判断签名是否存在
		String scriptRun = "";
		if(!TeeUtility.isNullorEmpty(zfryqm1)){
			scriptRun = "jacob.setBookMarksPicture(\"执法人员1签名\",\""+zfryqm1+"\",6,-1,15);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(zfryqm2)){
			scriptRun = "jacob.setBookMarksPicture(\"执法人员2签名\",\""+zfryqm2+"\",6,-1,15);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(gz)){
			scriptRun = "jacob.setBookMarksPicture(\"公章测试\",\""+gz+"\",6,-1,60);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(xcfzryj)){
			scriptRun = "jacob.setBookMarksPicture(\"现场负责人意见\",\""+xcfzryj+"\",6,-1,15);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(xcfzrqm)){
			scriptRun = "jacob.setBookMarksPicture(\"现场负责人签名\",\""+xcfzrqm+"\",6,-1,15);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		
		if(!TeeUtility.isNullorEmpty(dsrqz)){
			scriptRun += "jacob.setBookMarksPicture(\"当事人签名\",\""+dsrqz+"\",6,-1,20);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}
		if(!TeeUtility.isNullorEmpty(dsryj)){
			scriptRun += "jacob.setBookMarksPicture(\"当事人意见\",\""+dsryj+"\",6,-1,20);";
			response = jacobRequest.scriptRun(scriptRun, newWordId);
			newWordId = response.fileId;
		}

		return newWordId;
	}

}
