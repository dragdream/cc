...
function clickIt(siteId,channelId,type,hasPriv){
	var node;
	if(type=="site"){
		$("#frame").attr("src",contextPath+"/system/subsys/cms/channels.jsp?siteId="+siteId+"&channelId=0");
		node = zTreeObj.getNodeByParam("id",siteId+";site",null);
	}else if(type=="chnl"){
		if(hasPriv=="1"||hasPriv==1){
			$("#frame").attr("src",contextPath+"/system/subsys/cms/channels.jsp?siteId="+siteId+"&channelId="+channelId);
		}else{
			$("#frame").attr("src",contextPath+"/system/subsys/cms/tip2.jsp");
		}
...
<div id="layout">
	<div height="40" style="position: absolute;height: 40px;width: 100%;top:15px">
		<div style="vertical-align: middle;">
			<img id="img1" class = 'title_img' src="<%=contextPath %>/system/subsys/cms/image/lm.png">
