...
//返回
function back(){
	window.location.href=contextPath+"/system/subsys/project/myproject/index.jsp";
}
	...
	if(check()&&checkPNumIsExist()){
		...
		if(json.rtState){
			if(status==1){//保存  
				...
				if(isNoApprove()==0){//不是免审批人员
					window.location.href=contextPath+"/system/subsys/project/myproject/index.jsp?option=审批中";
				}else if(isNoApprove()==1){//免审批
					window.location.href=contextPath+"/system/subsys/project/myproject/index.jsp?option=办理中";
				}
			}
