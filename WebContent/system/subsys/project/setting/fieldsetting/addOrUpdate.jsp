...
function getInfoBySid(sid){
	var url=contextPath+"/projectCustomFieldController/getInfoBySid.action";
	var param={sid:sid};
	var json=tools.requestJsonRs(url,param);
	if(json.rtState){
		var data=json.rtData;
		bindJsonObj2Cntrl(data);
		if(data.fieldType=="下拉列表"){
			$("#codeTypeTr").show();
			if(data.codeType=="系统编码"){
				$("#sysCodeTr").show();
				$("#optionNameTr").hide();
				$("#optionValueTr").hide();
...
function commit(){
	if(check()){
		...
		var url=contextPath+"/projectCustomFieldController/addOrUpdate.action";
		var json=tools.requestJsonRs(url,param);
		if(json.rtState){
			$.MsgBox.Alert_auto("保存成功！");
			window.location.href=contextPath+"/system/subsys/project/setting/fieldsetting/index.jsp";
		}else{
...
<div>
 ...
 <table class="TableBlock_page">
    <tr>
       ...
       <td><input type="text" name="fieldName" id="fieldName" style="height: 23px;width: 350px" required/></td>
    </tr>
    <tr>
    ...
    <tr>
       <td style="text-indent: 15px">所属项目类型：</td>
       <td>
           <select id="projectTypeId" name="projectTypeId" style="width:200px;height: 23px">
           </select>
       </td>
    </tr>
    <tr>
       ...
       <td>
           <select id="fieldType" name="fieldType" onchange="changeFieldType()" style="width:200px;height: 23px"> 
    ...
    <tr style="display: none" id="codeTypeTr">
       ...
       <td>
           <input type="radio" name="codeType" id="xtbm"  value="系统编码" onclick="changeCodeType(1)" checked="checked"/>&nbsp;系统编码  &nbsp;&nbsp;
           <input type="radio" name="codeType"  value="自定义选项" onclick="changeCodeType(2)"/>&nbsp;自定义选项 
       </td>
    </tr>
    <tr style="display: none" id="sysCodeTr">
       <td style="text-indent: 15px">系统代码：</td>
       <td>
