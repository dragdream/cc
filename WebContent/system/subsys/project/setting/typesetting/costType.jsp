...
function getTypeList(){
	...
	if(json.rtState){
		...
		if(data.length>0){	
			...
			messageMsg("暂无费用类型！", "message" ,'info');	
		}
...
function addOrUpdate(sid){
	    var title="";
		var url=contextPath+"/system/subsys/project/setting/typesetting/addOrUpdateCostType.jsp?sid="+sid;
		if(sid>0){
			title="编辑费用类型";
		}else{
			title="新增费用类型";
...
function deleteBySid(sid){
	$.MsgBox.Confirm ("提示", "是否确认删除该费用类型?", function(){
		 var url=contextPath+"/projectCostTypeController/deleteBySid.action";
...
<body onload="doInit()">
  <div class="clearfix" style="padding-top: 5px;">
	<input   type="button" class="btn-win-white fr" value="增加费用类型" onclick="addOrUpdate(0);"/>
     ...
     <table style="border-collapse: collapse;border: 1px solid #f2f2f2;" width="100%" align="center" >
      <tbody id="tbody">
        <tr style="line-height:35px;border-bottom:2px solid #b0deff!important;background-color: #f8f8f8; ">
     	    ...
     	    <td style="width:33%;width:33.3333%;">费用类型</td>
