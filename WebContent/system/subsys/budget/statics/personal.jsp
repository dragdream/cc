...
function My_Submit(){
	  ...
	  window.location="<%=contextPath%>/system/subsys/budget/statics/personal.jsp?year="+year;
}
...
function getPersonalBudget(){
	var url = "<%=contextPath%>/teeBudgetReportController/getPersonalBudget.action?year="+year;
	var json = tools.requestJsonRs(url);
	if(json.rtState){
		var data = json.rtData;
		var budgetAmount=0;
		var costAmount=0;
		var html="<table class='TableBlock' style='width:80%;margin:0 auto;'><tr class='TableHeader'><td>人员</td><td>年份</td><td>预算</td><td>支出</td><td>剩余</td></tr>";
		for(var i=0;i<data.length;i++){
			budgetAmount = budgetAmount+data[i].AMOUNT;
			costAmount = costAmount+data[i].COSTAMOUNT;
			html+="<tr class='TableData'><td>"+data[i].USERNAME+"</td><td>"+data[i].YEAR+"</td><td align='right'>"+parseFloat(data[i].AMOUNT).toFixed(2)+"</td><td  align='right'>"+parseFloat(data[i].COSTAMOUNT).toFixed(2)+"</td><td  align='right'>"+parseFloat(data[i].AMOUNT-data[i].COSTAMOUNT).toFixed(2)+"</td></tr>";
		}
...
function exportPersonalBudget(){
	var url = "<%=contextPath%>/teeBudgetReportController/exportPersonalBudget.action?year="+year;;
...
<div class="moduleHeader" style='text-align:left;'>
			<div style='text-align:center;font-size:18px;'><strong>公司员工<%=year %>年预算执行情况统计</strong></div>
</div>
<form id="form1" name="form1" method="post">
	...
	<div id="control" style='margin:0 auto;text-align:center; height:28px;line-height:28px;width:"90%";margin-top:20px;'>
		<input type='button' class="btn btn-primary" value='导出' onclick='exportPersonalBudget();'/>
	</div>
	<br/><br/>
</form>
</body>
