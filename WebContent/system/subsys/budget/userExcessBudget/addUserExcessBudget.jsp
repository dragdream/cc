...
function checkForm(){
	...
	if(!userId){
		top.$.jBox.tip("请选择人员！",'info',{timeout:1500});
		return false;
	}
	...
	if(returnPrc == '1'){
		top.$.jBox.tip("该用户年度预算已存在！",'info',{timeout:1500});
		return false;
	}
	return true;
}
...
var userId=$("#userId").val();
//alert(userId);
		var url = contextPath+"/userExcessBudgetController/adduserExcess.action?userId="+userId;
		var para =  tools.formToJson($("#form1"));
		var jsonRs = tools.requestJsonRs(url,para);
		if(jsonRs.rtState){
		//	toReturn();
