...
<div id="ntkoDiv">
	<button class="btn-win-white" onclick="downLoadDoc()"
		id="zwDownLoad">
		<i class="glyphicon glyphicon-print"></i>&nbsp;下载
	</button>
...
with (TANGER_OCX_OBJ.ActiveDocument) {
	...
	if (cmd==5)
	{
		...
		if (pp.display()==-1)
		{
			...
			var j = tools.requestJsonRs(contextPath+"/doc/getDeliveryInfo.action?uuid="+parent.uuid);
			...
			else
			{
				pp.Execute();
				// +1
				var j = tools.requestJsonRs(contextPath+"/doc/updateDeliveryPrintedNum.action?uuid="+parent.uuid+"&&pNum="+pp.NumCopies);
			}
		}
