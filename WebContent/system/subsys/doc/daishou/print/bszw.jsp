...
<div id="aipDiv" style="">
	...
	if(data1!=null){
		...
		AIP.SetValue("SET_CURRENT_COOKIE","COOKIE:"+document.cookie);
 		AIP.LoadFile("<%=request.getProtocol().toLowerCase().contains("https")?"https":"http"%>://<%=request.getServerName()%>:<%=request.getServerPort()%>/attachmentController/downFile.action?id="+data1);
		aipId = data1;
	} else {
	...
	<!-- 版式正文打印 控制打印份数  -->
    <!-- JSNotifyBeforeAction通过这个事件去接收用户打印操作
    HWPostil1.JSValue = 0;  这个接口是终止用户的打印操作
    HWPostil1.JSEnv = 0;  等于1触发时间，等于0是不触发事件
    HWPostil1.PrintDocEx然后根据用户输入的份数，判断权限，执行自己的打印输出
	HWPostil1.GrayData(31);  这个是文档脱密
    HWPostil1.GrayData(0);  恢复红头文件 
     -->	
 	<SCRIPT LANGUAGE=javascript FOR=TeeHWPostil EVENT=JSNotifyBeforeAction(lActionType,lType,strName,strValue)>
      ...
      if(lActionType==1){//判断是否是打印
		var jsonObj = tools.requestJsonRs(contextPath+"/doc/getDeliveryInfo.action?uuid="+parent.uuid);
		...
		if(lType>pNum1){
			...
			//后台修改打印份数
			var j = tools.requestJsonRs(contextPath+"/doc/updateDeliveryPrintedNum.action?uuid="+parent.uuid+"&&pNum="+lType);
