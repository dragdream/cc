...
%>
	...
	function query(){
		...
		datagrid = $('#datagrid').datagrid({
			url:'<%=contextPath%>/doc/myDocSend.action',
			toolbar : '#toolbar',
	...
	function deleteObjFunc(ids){
		$.MsgBox.Confirm ("提示", "确定要删除所选中记录？",function(){
				var url = contextPath + "/doc/deleteObjById.action";
				var para = {sids:ids};
	...
	function takeBackFunc(ids){
		$.MsgBox.Confirm ("提示", "确定要收回所选中记录？",function(){
				var url = contextPath + "/doc/takeBackObjById.action";
				var para = {sids:ids};
				var json = tools.requestJsonRs(url,para);
				if(json.rtState){
	...
	function reSendFunc(id){
		 var url = contextPath + "/system/subsys/doc/feedback/reSendFeedback.jsp?uuid=" + id;
		 bsWindow(url ,"公文分发",{width:"600",height:"300",
		  ...
		  ,submit:function(v,h,f,d){
		    ...
		    if(v=="确定"){
		    	cw.doSaveOrUpdate(function(){
		    	  parent.$.MsgBox.Alert_auto("公文已成功下发！");
	 			  datagrid.datagrid('reload');
	 			  d.remove();
	 			  //BSWINDOW.modal("hide");
	 			});
	...
	function detail(id){
		var url = contextPath+"/system/subsys/doc/feedback/feedback_detail.jsp?uuid="+id;
...
<div class="fl" style="position:static;">
		<img id="img1" class = 'title_img' src="<%=contextPath %>/common/zt_webframe/imgs/gwgl/fwfk/icon_fwfk.png">&nbsp;&nbsp;
		<span class="title">发文反馈</span>
</div>
...
<form id="form" name="form1" style="padding:2px">
	<table style="width:100%">
		<tr style="height: 30px;">
			...
			<td class="TableData" style="width: 250px;font-family: MicroSoft YaHei;"><input type="text" name="bt"  style="height: 25px;width: 200px;font-family: MicroSoft YaHei;" class="BigInput"/></td>
			<td class="TableData" width="50px;">文号：</td>
			<td class="TableData" style="width: 220px;font-family: MicroSoft YaHei;"><input type="text" name="zh"  style="height: 25px;width: 200px;font-family: MicroSoft YaHei;" class="BigInput"/></td>
