...
function doInit(){
	$("#group").group();
	changePage("<%=contextPath%>/system/subsys/attendAssign/manage/comfireNo.jsp");
	getPersonTree();
...
function personOnClick(event, treeId, treeNode) {
	...
	if(uuid.split(";").length == 2 && uuid.split(";")[1] == 'dept'){
		...
		changePage(contextPath + "/system/subsys/attendAssign/manage/query.jsp?personId="+uuid.split(";")[0]);
	}
};
