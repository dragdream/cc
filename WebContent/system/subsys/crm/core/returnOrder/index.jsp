...
//新建退货单
function add(){
	var url = contextPath+"/system/subsys/crm/core/returnOrder/addOrUpdate.jsp";
	location.href=url;
}
...
<body onload="doInit();"  style="overflow:hidden;padding-left:10px;padding-right:10px;">
 ...
 <div class="titlebar clearfix" >
		<img class = 'tit_img' style="margin-right: 10px;" src="<%=contextPath%>/system/subsys/crm/img/icon_thd.png">
		<p class="title">退货单</p>
		<ul id = 'tab' class = 'tab clearfix' style='display:inline-block;'>
		</ul>
	<div class="right fr clearfix" style="padding-top: -1px;">
      <input type="button" value="新建退货单" class="btn-win-white" onclick="add();"/>
	 ...
	 <form id="form1" class='ad_sea_Content'>
       <table class="TableBlock" width="100%"  style="font-size:12px" id="searchTable">
          <tr>
            <td width="10%">退货单号：</td>
             <td width="40%">
                <input  style="width: 300px;" type="text" name="returnOrderNo" id="returnOrderNo"/>
             </td>
             <td width="10%">
				所属客户：
			</td>
          ...
          <tr>
            <td>退单日期：</td>
             <td>
                 <input type="text" style="width: 180px;" requried id='returnTimeDesc' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name='returnTimeDesc' class="Wdate BigInput" />
             </td>
              <td>状态：</td>
             <td>
                <select style="width: 180px;" id="returnOrderStatusDesc" name="returnOrderStatusDesc">
           ...
           <tr>
            <td width="10%">退货原因：</td>
             <td width="40%">
              <select class="BigSelect" id='returnReason' name='returnReason' style="width:180px;">
					<option value='0'>全部</option>
				</select>
             </td>
             <td width="10%">
...
<script type="text/javascript">
var customerId = "<%=sid%>";
 $.addTab("tab","tab-content",[{title:"全部",url:contextPath+"/system/subsys/crm/core/returnOrder/allReturnOrderList.jsp?type=1"},
                              {title:"我负责的",url:contextPath+"/system/subsys/crm/core/returnOrder/allReturnOrderList.jsp?type=2"},
                              {title:"我下属负责的",url:contextPath+"/system/subsys/crm/core/returnOrder/allReturnOrderList.jsp?type=3"},
                              ]); 
