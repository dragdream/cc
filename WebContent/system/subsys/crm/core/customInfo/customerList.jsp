...
function doInit(){
	...
	datagrid = $('#datagrid').datagrid({
		url:contextPath+'/TeeCrmCustomerInfoController/datagrid.action',//type=3代表公海客户，type=2 代表所有的客户，type =1 共享客户  没有type代表我的客户
		pagination:true,
		singleSelect:false,
		...
		columns:[[
			...
			{field:'type',title:'客户性质',width:70,formatter:function(data){
				if(data==1){
					return "客户";
				}else{
					return "供应商";
			...
			{field:'customerSourceDesc',title:'客户来源',width:80},
		...
		onLoadSuccess: function(data){//加载完毕后获取所有的checkbox遍历
            if (data.rows.length > 0) {
                ...
                for (var i = 0; i < data.rows.length; i++) {
                    //根据operate让某些行不可选
                    if (data.rows[i].managePersonId != loginPersonId) {
...
function add(){
	var url = contextPath+"/system/subsys/crm/core/customInfo/addOrEditCustomer.jsp";
	location.href=url;
}

function edit(sid){
	datagrid.datagrid("unselectAll");
	var url = contextPath+"/system/subsys/crm/core/customInfo/addOrEditCustomer.jsp?sid="+sid;
	location.href=url;
}
