...
function getInfoById(id){
		...
		var url ="<%=contextPath%>/crmDeptTargetController/getDeptTargetById.action";
		var para = {sid :id};
...
function save(){
	if(checkForm()){
		var url = "<%=contextPath %>/crmDeptTargetController/addOrUpdate.action";
		var para =  tools.formToJson($("#form1"));
		var jsonRs = tools.requestJsonRs(url,para);
		if(jsonRs.rtState){
			goback();
			$.jBox.tip("保存成功!",'success',{timeout:1500});
		}else{
			$.jBox.tip(jsonRs.rtMsg,'error',{timeout:1500});
		}
	}
...
//返回
function goback(){
	window.location = contextPath+"/system/subsys/crm/core/target/deptTargetList.jsp?year="+year;
}
		...
		<table class="TableBlock" style="font-size:12px;margin:0 auto;">
			...
			<tr>
				...
				<td class="TableData" width="300px">
				    ...
				    <!-- <select class="BigInput easyui-validatebox" required name="year" style="width:100px;">
			...
			<tr>
				...
				<td class="TableData">
					...
					<input id="deptName" name="deptName"  type="text" class="BigInput  readonly" required readonly/>
