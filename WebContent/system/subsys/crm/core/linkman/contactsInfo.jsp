...
function addMenu(data){
	var str;
	$(".btn-content").empty();
	if(managerPerId==loginPersonId){//负责人是当前登录者
		if(data == "正常"){
			$(".btn-group").show();
	 ...
	 var jsonRs = tools.requestJsonRs(url,null);
		if(jsonRs.rtState){
			$.MsgBox.Alert_auto("操作成功！",function(){
				parent.window.location.href = "<%=contextPath%>/system/subsys/crm/core/order/index.jsp?type="+type;
...
//编辑
function edit(){
	window.location.href = "<%=contextPath%>/system/subsys/crm/core/linkman/addOrEditContactUser.jsp?sid=" + sid+"&customerName=<%=customerName%>";
}

//返回
function back(){
	parent.window.location.href = "<%=contextPath%>/system/subsys/crm/core/linkman/contactsList.jsp";
}
...
<div class="topbar clearfix" id="toolbar">
   <div class="fl" style="position:static;">
		<img id="img1" class = 'title_img' src="<%=contextPath %>/system/subsys/crm/img/icon_lxrxq.png">
