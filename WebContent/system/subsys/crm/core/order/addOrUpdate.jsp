...
function save(){
	...
	if(sid>0){
		if(check()){
		    ...
		    var json=tools.requestJsonRs(url,param);
			    if(json.rtState){
				      ...
				      window.location.href=contextPath+"/system/subsys/crm/core/order/orderDetial.jsp?sid="+sid+"&customerName="+cusName; 
		...
		if(check()){
			...
			var param2=tools.formToJson("#form1");
			var itemList2 = getProductItem();//产品明细
			var str2 = tools.jsonArray2String(itemList2);
			//alert(str2);
			param2["productsList"] = str2;
			var json2=tools.requestJsonRs(url,param2);
		        if(json2.rtState){
		    	     parent.$.MsgBox.Alert_auto(json2.rtMsg,function(){
			         window.location.href=contextPath+"/system/subsys/crm/core/order/index.jsp";
		    	 });
		    	 };	
		}
...
function addSaleFollowProcuctItem(item , optType){
	...
	if(item){
		...
		priductsModel_value = item.productsTypeName;
	...
	//产品编号
	var product_no_input = $("<input type='text' readonly name='a1' required style='line-height:20px;width:120px;'>").addClass("").val(product_no_value);
	var productNo=$("<td required>").append(product_no_input);
/* 	product_no_input.validatebox({ 
		required:true 
	});  */
	//产品名称 
	var product_name_input = $("<input readonly type='text' name='a2' required style='line-height:20px;width:120px;'>").addClass("").val(product_name_value);
...
/* 	product_name_input.validatebox({ 
	...
	//产品类型
	var priductsModel=$("<td>").append($("<input readonly type='text' required style='line-height:20px;width:120px;'>").addClass("").val(priductsModel_value));
	
	var units=$("<td>").append($("<select>").append(CRM_PRODUCTS_UNITS_STR).addClass("").val(units_value));
	
	//数量
	var item_number_input = $("<input type='text' required style='line-height:20px;width:60px;'>").addClass("")
...
/* 	item_price_input.validatebox({ 
	...
	//合计
	var item_total_amount_input = $("<input type='text' readonly required style='line-height:20px;width:130px;'>").addClass("")
...
<div class="topbar clearfix" id="toolbar">
   <div class="fl" style="position:static;">
		<img id="img1" class = 'title_img' src="<%=contextPath %>/system/subsys/crm/img/icon_xjdd.png">
  ...
  <table class="TableBlock_page" width="60%" align="center">
    ...
    <table class="TableBlock_page" width="60%" align="center">
	    ...
	    <tr>
			...
			<td>
				<input style="height: 23px;width: 350px;" id='receiverName' name='receiverName' class="BigInput" type='text' readonly="readonly"/>
	    ...
	    <tr>
          <td class="TableData" width="150" style="text-indent:15px">收货人电话：</td>
		   <td class="TableData">
				<input type="text" style="width: 350px;" requried id="receiverTelephone" name="receiverTelephone" class="BigSelect"></select>
		   </td>
        </tr>
          <tr>
          <td class="TableData" width="150" style="text-indent:15px">交货日期 ：</td>
			<td>
				<input readonly="readonly" type="text" style="width: 200px;" requried id='transactionsTimeDesc' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name='transactionsTimeDesc' class="Wdate BigInput" />
			</td>
        </tr>
   </table>
   <table class="TableBlock_page" width="60%" align="center">
