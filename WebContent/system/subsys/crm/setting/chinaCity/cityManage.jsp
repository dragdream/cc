...
function getInfoList(){
	datagrid = $('#datagrid').datagrid({
		url:contextPath+"/chinaCityController/getCityListByProvinceCode.action?provinceCode=<%=provinceCode%>",
		pagination:true,
		singleSelect:false,
		...
		columns:[[
			...
			{field:'cityName',title:'城市名称',width:100,formatter:function(value,rowData,rowIndex){
...
function countyManageFunc(sid){
  var url = contextPath + "/system/subsys/crm/setting/chinaCity/countyManage.jsp?provinceCode=<%=provinceCode%>&cityCodeStr=" + sid;
  location.href = url;
}
 ...
 */
 function addOrUpdateInfo(sid){
   ...
   var url = contextPath + "/system/subsys/crm/setting/chinaCity/addOrUpdateCity.jsp?provinceCode=<%=provinceCode%>&sid=" + sid;
...
function deleteObjFunc(ids){
  $.jBox.confirm("确定要删除所选中记录？","确认",function(v){
		if(v=="ok"){
			var url = contextPath + "/chinaCityController/deleteCityObjById.action";
			var para = {sids:ids};
...
function getCityTitle(cityCode){
	...
	if(cityObj){
		...
		allProvinceStr +=  "<span style='color:#000000'>" + cityObj.cityName + "</span>";
...
<body onload="doInit()" style="overflow:hidden;font-size:12px">
	...
	<div id="toolbar">
		...
		<div style="text-align:left;">
			<button class="btn btn-primary btn-xs" onclick="addOrUpdateInfo(0)">添加城市</button>&nbsp;&nbsp;
