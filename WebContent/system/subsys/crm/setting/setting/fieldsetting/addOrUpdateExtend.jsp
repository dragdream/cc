...
function getInfoBySid(sid){
	var url=contextPath+"/CustomerFieldController/getInfoBySid.action";
	var param={sid:sid};
...
function commit(){
	...
	if(check()){
		...
		if(sid>0){
				url = contextPath+"/CustomerFieldController/updateField.action?sid="+sid;
		}else{
			 url=contextPath+"/CustomerFieldController/addOrUpdate.action";
		}
		var json=tools.requestJsonRs(url,param);
		if(json.rtState){
			parent.$.MsgBox.Alert_auto("保存成功！",function(){
				window.location.href=contextPath+"/system/subsys/crm/setting/setting/fieldsetting/index.jsp";
			});
...
<div class="topbar clearfix" id="toolbar">
   <div class="fl" style="position:static;">
		<img id="img1" class = 'title_img' src="<%=contextPath %>/system/subsys/crm/img/icon_tjzd.png">
 ...
 <table class="TableBlock_page">
    ...
    <tr>
     ...
     <tr style="display: none" id="codeTypeTr">
       ...
       <td>
           <input type="radio" name="codeType" id="xtbm"  value="CRM系统编码" onclick="changeCodeType(1)" checked="checked"/>&nbsp;系统编码  &nbsp;&nbsp;
           <input type="radio" name="codeType"  value="自定义选项" onclick="changeCodeType(2)"/>&nbsp;自定义选项 
       </td>
    </tr>
    <tr style="display: none" id="sysCodeTr">
       <td style="text-indent: 15px">CRM系统代码：</td>
       <td>
