...
function DoHandSign(key) {
  if (TANGER_OCX_bDocOpen) {
    if (TANGER_OCX_OBJ.IsNTKOSecSignInstalled()){
      ...
      try{
        TANGER_OCX_OBJ.DoHandSign(TANGER_OCX_user, 0, 0x00000000, 2, 100, 50,
            null, key);
      }catch(e){}
    }
  }
...
function TANGER_OCX_AddDocHeader(attrId,attrName) {
  try {
    ...
    curSel.WholeStory();
    curSel.Cut();
    //addtemplefromlocalsetfont(URL , "zhengwen");
    ...
    try {
      ...
      TANGER_OCX_OBJ.ActiveDocument.Bookmarks.Add(BookMarkName, saverange);
       ...
       bkmkObj = TANGER_OCX_OBJ.ActiveDocument.BookMarks(BookMarkName);
...
function addtemplefromlocalsetfont(url,bookmark){
  obj = TANGER_OCX_OBJ;
  var sel=obj.ActiveDocument.Application.Selection;
  if(obj.ActiveDocument.BookMarks.Exists(bookmark)){
    sel.GoTo(-1,0,0,bookmark);
sel.TypeText(" ");
var shuq1=obj.ActiveDocument.Bookmarks(bookmark);
var rang=obj.ActiveDocument.Range(shuq1.Start+1,shuq1.End+1);
rang.Select();
var shuq2=obj.ActiveDocument.Bookmarks.Add("temp",rang);
shuq1.Select();
obj.AddTemplateFromURL(url);
var text=obj.ActiveDocument.Range(shuq1.End,shuq2.Start);
text.Font.Size = 16;
...
//  TANGER_OCX_OBJ.caption = "Office文档在线编辑";
...
//  TANGER_OCX_EnableFileSaveAsMenu(false);
  try {
    ...
    TANGER_OCX_strOp = $("#TANGER_OCX_op")[0].innerHTML;
    TANGER_OCX_filename =  $("#TANGER_OCX_filename")[0].innerHTML;
    TANGER_OCX_attachName = $("#TANGER_OCX_attachName")[0].innerHTML;
    TANGER_OCX_attachURL = $("#TANGER_OCX_attachURL")[0].innerHTML;
    TANGER_OCX_user =  $("#TANGER_OCX_user")[0].innerHTML;
    ...
    if(TANGER_OCX_strOp){
    	...
    	}else if(TANGER_OCX_strOp =="5" ){
    		
    	}else if(TANGER_OCX_strOp =="6" ){
    		
    	}else if(TANGER_OCX_strOp =="7" ){
    		 info = "阅读文档";
    	        if (TANGER_OCX_attachURL) {
...
function TANGER_OCX_SaveDoc(op_flag) {
  ...
  paraObj.PARA = "";
  paraObj.FFN = "";
  var _attachmentName = $("#attachmentName")[0].value;
  try {
  //  if (!TANGER_OCX_doFormOnSubmit())
   //   return;
 //   $('#docSize').val(TANGER_OCX_OBJ.DocSize);
  ...
  //  }
   ...
   // }
    ...
    case "4":
    //  lockRef();
      retStr = TANGER_OCX_OBJ.SaveToURL(window.contextPath+"/ntko/updateFile.action", "file", "",
    		  _attachmentName, "form1");
      if (op_flag == 1) {
        TANGER_OCX_bDocOpen = false;
        CloseWindow();
      }
...
function SaveAsPDFFile(IsPermitPrint, IsPermitCopy) {
  try {
    ...
    if (err.number == -2147467259)
      alert("该功能需要软件PDFCreator支持\n请下载安装 http://www.go2oa.com/system/PDFCreator-0_9_5_setup.exe");
  }
}
...
function fileExits(param,type){
  ...
  var url = contextPath + "" + type + "&" + param;
 ...
 */
 function saveZhaiYao(){
   ...
   var rtJson = getJsonRs(contextPath + "", queryParam);
   if (rtJson.rtState == "0") {
     alert("摘要保存成功!");
   }else {
     alert("摘要保存失败!");
