...
function load_init()
{
   ...
   jQuery("body").on("click",'div.panel-group .list-group-item',function(){
//   jQuery('div.panel-group .list-group-item').live('click', function(){
   ...
   */
   //右侧全部添加点击、鼠标hover事件
   jQuery("body").on("click",'#addAll',function(){
   ...
   */
   //右侧全部删除点击、鼠标hover事件
   jQuery("body").on("click",'#removeAll',function(){
   ...
   */
   ...
   //右侧全体部门点击、鼠标hover事件
   jQuery("body").on("click",'div.right .block-right-alldept',function(){
   ...
   jQuery("body").on("mouseenter",'div.right .block-right-goback',function(){
   //jQuery('div.right .block-right-goback').live('mouseenter', function(){
      jQuery(this).addClass('goback-hover');
   });
   jQuery("body").on("mouseleave",'div.right .block-right-goback',function(){
...
function trigger_callback(type, args ){
	if(type && xparent[type]){
        xparent[type](args[0],args[1],args[2],args[3],args[4],args[5]);
    }
}
...
function add_item(item_id, item_name , extend)
{
   ...
   to_id_field_value+="";
   to_name_field_value+="";
   if(to_id_field_value.charAt(to_id_field_value.length-1)==','){
	   to_id_field_value = to_id_field_value.substring(0, to_id_field_value.length-1);
   }
   if(to_name_field_value.charAt(to_name_field_value.length-1)==','){
	   to_name_field_value = to_name_field_value.substring(0, to_name_field_value.length-1);
   }
...
function getPersonByRole(roleId,roleName){
	var url = contextPath +  "/orgSelectManager/getSelectUserByRole.action";
	if(!userFilter){
		userFilter = "0";
	}
	if(!deptFilter){
	...
	var para = {roleId:roleId,userFilter:userFilter,deptFilter:deptFilter,moduleId:moduleId,privNoFlag:privNoFlag};
...
function getPersonByUserGroup(id,name){
	var url = contextPath + "/orgSelectManager/getPersonByUserGroup.action";
	if(!userFilter){
		userFilter = "0";
	}
	if(!deptFilter){
	...
	var para = {userGroupId:id,userFilter:userFilter,deptFilter:deptFilter,moduleId:moduleId,privNoFlag:privNoFlag};
	var jsonRs = tools.requestJsonRs(url,para);
	if(jsonRs.rtState){
		var dataList = jsonRs.rtData;
...
function dataLoad(dataList,name , type){
	 ...
	 if(dataList.length > 0){
		 $("#dept_item_0").append("<a   class='list-group-item-header' style='padding:8px 15px;cursor:pointer'><input type='checkbox' id='isCheckAll' onclick='checkAll(this)'/>" + name + "</a>");
...
function checkAll(){
//    alert($("#isCheckAll").attr("checked"));
	if($("#isCheckAll").attr("checked")=="checked"){
		$("#dept_item a.list-group-item").each(function(i,obj){
			var id = $(obj).attr("item_id");
			var name = $(obj).attr("item_name");
			jQuery($(obj)).addClass('active');
			add_item(id,name);
		});
		//$("#dept_item a[class=list-group-item]").attr("class","list-group-item active");
	}else{
		$("#dept_item a.list-group-item").each(function(i,obj){
			var id = $(obj).attr("item_id");
			var name = $(obj).attr("item_name");
...
function getGroup(){ 
	if(!isUserGroupClick){
		var url = contextPath + "/userGroup/getPublicAndPersonalUserGroup.action";
		var para = {};
		var jsonRs = tools.requestJsonRs(url,para);
		if(jsonRs.rtState){
		  	var dataList = jsonRs.rtData;
