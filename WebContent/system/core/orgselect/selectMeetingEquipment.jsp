...
function doInit(){
	...
	var meetingEquipmentArray = parentWindowObj["meetingEquipment"];
	if (meetingEquipmentArray && (meetingEquipmentArray.length == 2 ||  meetingEquipmentArray.length == 3) ) {
	    var roleCntrl = meetingEquipmentArray[0];
	    var roleDescCntrl = meetingEquipmentArray[1];
	    RoleId = parentWindowObj.document.getElementById(roleCntrl);
	    RoleName = parentWindowObj.document.getElementById(roleDescCntrl);
	}else {
	    RoleId = parentWindowObj.document.getElementById("meetingEquipment");
	    RoleName = parentWindowObj.document.getElementById("meetingEquipmentDesc");
	}
	to_id_field = RoleId;
	to_name_field = RoleName;
	var url = contextPath +  "/orgSelectManager/getMeetingEquipment.action";
	...
	if(jsonRs.rtState){
		...
		for (var i = 0;i<dataList.length; i++) {
			var roleId = dataList[i].sid;
			var roleName = dataList[i].equipmentName;
			$("#dept_item_0").append("<a class='list-group-item active' href='javascript:void(0);' item_id='"+roleId+ "' item_name='"+roleName+"'><h6 class='list-group-item-heading'>"+ roleName+"</h6></a>");
			temp = false;
		}
		if(temp){
			 $("#dept_item_0").append("<div align='center' class='emptyClass' style='padding-top:5px;'><h6>无相关设备！</h6></div>");
		}
	 	load_init();  
	 	load_init_item(); 
	    //默认加载角色选中状态
...
<div class="main-block" id="block_dept" style="display:block;">
    <div class="right single" align="center" style="margin:10px 10px 10px 10px;left:5px;padding-top:10px" id="dept_item">
	   <div class="block-right" id="dept_item_0">
		   <a href='javascript:void(0);' class='list-group-item-header'>选择设备</a>
