...
function getDeptTree(){
	var url = "<%=contextPath %>/orgSelectManager/getSelectDeptTreePost.action?moduleId="+moduleId;
	var config = {};
	if(isSingle == '1'){
		 config = {//单选
...
function doSearch(name){
	var url = contextPath +  "/orgSelectManager/selectPostDeptByDeptName.action";
	var para = {dept:name};
	var jsonRs = tools.requestJsonRs(url,para);
	if(jsonRs.rtState){
		var dataList = jsonRs.rtData;
