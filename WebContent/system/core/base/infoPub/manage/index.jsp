...
function changePage(index){
	switch(index){
	case 1:
		$("#frame0").attr("src",contextPath+"/system/core/base/infoPub/manage/manageInfoList.jsp?infoType=<%=infoType%>");
		break;
	case 2:
		$("#frame0").attr("src",contextPath+"/system/core/base/infoPub/manage/addInfo.jsp?infoType=<%=infoType%>");
		break;
	case 3:
		$("#frame0").attr("src",contextPath+"/system/core/base/infoPub/manage/queryInfo.jsp?infoType=<%=infoType%>");
		break;
	}
}
...
<div class="base_layout_left" style="text-align:center;width:150px;" >
	...
	<ul class="my_nav" id="group">
		<li class='active' onclick="changePage(1)"><%=infoTypeDesc %>管理</li>
		<li onclick="changePage(2)">新建<%=infoTypeDesc %></li>
