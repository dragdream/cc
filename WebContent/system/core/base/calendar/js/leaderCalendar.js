...
function createMonthTable(dateList , currDateStr , userId){
	...
	for ( var i = 0; i < dateList.length ; i++) {
		...
		for ( var j = 0; j < dateArray.length; j++) {
			...
			if(isCurrMonth){
				tdStr = tdStr + 
			      "<td id='td_"+userId +"_" + date.simpDate +"' ondblclick='toNewCalendarByDay("+userId+",\""+(date.simpDate)+"\")' valign='top'   style='min-height:80px;height:80px;"+tdClassStyle+"' title='双击建立日事务'><div align='right' class='TableContent'  title='' style='cursor:pointer;width: 100%;height:20px;'><font color='blue'  align='right'><b >"+day+"</b></font></div></td>";
			}else{
				tdStr = tdStr + 
...
function setCalendarView(view , currView){
	...
	if(view == 'day'){
		window.location.href = contextPath + "/system/core/base/calendar/leader/day.jsp?deptId=" + deptId + "&dateStr=" + dateStr + "&fromView=" + currView;
	}else if(view == 'week'){
		window.location.href = contextPath + "/system/core/base/calendar/leader/week.jsp?deptId=" + deptId + "&dateStr=" + dateStr + "&fromView=" + currView;
	}else if(view == 'month'){
		
		window.location.href = contextPath + "/system/core/base/calendar/leader/month.jsp?deptId=" + deptId + "&dateStr=" + dateStr + "&fromView=" + currView;
	}
...
function toNewCalendarByDay(userId , date )
{
	...
	var url = contextPath + "/system/core/base/calendar/leader/addOrUpdate.jsp?userId=" + userId + "&date=" + date + "&userName=" + encodeURIComponent(userName);
...
{
	var url = contextPath + "/system/core/base/calendar/leader/addOrUpdate.jsp?sid=" + id;
...
function selectCalendar(){
	window.location.href = contextPath + "/system/core/base/calendar/leader/query.jsp";
}
