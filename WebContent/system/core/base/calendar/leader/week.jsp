...
<script type="text/javascript" src='<%=contextPath%>/system/core/base/calendar/js/leaderCalendar.js'></script>
...
function createNullCalendarTr(value,name){
  	...
  	if(!$("#tr_" + value)[0]){//如果不存在
  	    ...
  	    for(var i = 1;i<8 ; i++){
  	        ...
  	        tmp = tmp  +"<td id='td_"+value+"_"+ dateList[i-1].simpDate +"'  class='TableData' title='双击建立事务' ondblclick='toNewCalendarByDay("+value+",\""+(dateList[i-1].simpDate)+"\")' style='"+tdClassStyle+"'></td>";
  	    }
