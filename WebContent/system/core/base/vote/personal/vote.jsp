...
function doInit(){
	...
	for(var i=0;i<subjects.length;i++){
		...
		html.push("</tr>");
		html.push("<tr>");
		html.push("<td class='TableData' style='padding-top:5px;border-left:1px solid #ccc;border-right:1px solid #ccc;border-bottom:1px solid #ccc;padding-left:20px'  subject='"+subjectItem.subject+"' type='"+subjectItem.type+"' min='"+subjectItem.min+"' max='"+subjectItem.max+"' required0='"+subjects[i].required+"'>");
		if(subjectItem.type=='0'){//多选
			...
			for(var j=0;j<voteItems.length;j++){
				...
				html.push("<input style='margin-top:3px;'  class='BigCheckbox' type='checkbox' itemId="+itemId+" id='cb"+itemId+"' name='cb"+itemId+"'/>&nbsp;<label  style='width: 95%;margin-top:3px;' for='cb"+itemId+"'>"+name+"</label><br/>");
			}
			...
			for(var j=0;j<voteItems.length;j++){
				var name = voteItems[j].name;
				var itemId = voteItems[j].itemId;
				html.push("<input style='' type='radio' id='rd"+itemId+"' itemId="+itemId+" name='vote"+voteId+"'/>&nbsp;<label  style='width: 95%;' for='rd"+itemId+"'>"+name+"</label><br/>");
			}
		}else if(subjectItem.type=='2'){//文本框
			var itemId = subjectItem.voteItem.itemId;
			var name = subjectItem.voteItem.name;
			html.push("<input  class='BigInput' type='text' itemId="+itemId+" name='ipt"+itemId+"' id='ipt"+itemId+"' style='width:520px;'/>");
		}else if(subjectItem.type=='3'){//多行
			var itemId = subjectItem.voteItem.itemId;
			var name = subjectItem.voteItem.name;
			html.push("<textarea class='BigTextarea' itemId="+itemId+" name='txt"+itemId+"' id='txt"+itemId+"' style='width:520px;height:150px'></textarea>");
		...
		if(subjectItem.ifContent==1){
			html.push("<div style='margin-top:10px;color:red'>");
			html.push("<b>说明：</b><br/>");
...
function vote(){
	...
	if(anonymity=='1'){
		para["anonymity"] = 1;
		  ...
		  $.MsgBox.Confirm ("提示", "是否确认匿名投票？", function(){
		...
		$.MsgBox.Confirm ("提示", "是否确认投票？", function(){
			var url = contextPath+"/voteManage/savePersonalVote.action";
			var json = tools.requestJsonRs(url, para);
			if(json.rtState){
				var checkFlag = json.rtData.checkFlag;
...
<body onload="doInit()" id="body" style="margin: 10px;overflow-x:hidden;padding-top: 10px">
  <div id="toolbar" class="clearfix" >
      <div class="fl" style="position:static;">
		<b><span class="">投票&nbsp;———&nbsp;<span id="subject"></span></span></b>   
	  </div>
	   <div id="btns" class="fr right" style="margin-right:20px">
	       <button class="btn-win-white" onclick="vote()">提交</button>
		   &nbsp;&nbsp;
...
<div class="moduleHeader" >
	<b>投票&nbsp;-&nbsp;<span id="subject"></span></b>
</div>
<div id="content" style="margin-left:30px; margin-bottom:10px; font-size:14px;width: 90%;"></div>
	<center>
	<table class="TableBlock" id="tb" style="width:95%;font-size:12px">
	</table>
	<br/>
		<button class="btn btn-default" onclick="vote()">提交</button>
		&nbsp;&nbsp;
