...
function add(){
	var url = contextPath+"/system/core/base/pm/manage/zs_add.jsp?humanDocSid=<%=humanDocSid%>";
	bsWindow(url,"添加证书信息",{width:"700",height:"300",submit:function(v,h){
...
function edit(sid){
	datagrid.datagrid("unselectAll");
	var url = contextPath+"/system/core/base/pm/manage/zs_edit.jsp?sid="+sid;
	bsWindow(url,"修改证书信息",{width:"700",height:"300",submit:function(v,h){
		var cw = h[0].contentWindow;
		if(cw.commit()){
			datagrid.datagrid("unselectAll");
			datagrid.datagrid("reload");
	...
	<form id="form1" name="form1">
		<div style="margin-top:10px;display:none" id="searchDiv">
					<table class="SearchTable" style="text-align:left;">
						<tr>
							<td class="SearchTableTitle">证书编号：</td>
							<td>
								<input class="BigInput" type="text" id = "certCode" name='certCode' style="width:250px;"/>
							</td>
							<td class="SearchTableTitle">证书名称：</td>
							<td>
								<input class="BigInput" type='text' id='certName' name='certName' style="width:165px;"/>
							</td>
						</tr>
						<tr>
							<td class="SearchTableTitle">证书类型：</td>
							<td>
									<select class="BigSelect" id="certType" name="certType">
										<option value="全部">全部</option>
									</select>
							</td>
							<td class="SearchTableTitle">证书状态：</td>
							<td>
								<select class="BigSelect" id='certAttr' name='certAttr' style="width:180px;">
									<option value="全部">全部</option>
								</select>
