...
function doSearch(){
	if(checkForm()){
		var url = contextPath + '/diaryController/doSearchShare.action';
		...
		tools.requestJsonRs(url,para,true,function(json){
			if(json.rtState){
				...
				$(".j_content_list").each(function(i,obj){
					if($(obj).html()==""){
						$(obj).append("<p style='padding-left:20px;font-size:14px;'>无共享日志内容！</p>");
					}
				});
				//$.jBox.closeTip();
			}
...
<div style="padding:10px;min-width:800px;background-color:#f5f6f7;">
	...
	<table style="width:700px;font-size:12px;background-color:transparent;margin-left:62px;" align="center" class="TableBlock">
		<tr style='border:none;'>
			...
			<td style="width: 150px;" class="TableData"><input id="title" name="title" type="text" class="BigInput" style="width:120px;height:25px;" /></td>
			<td class="TableData ">类型：</td>
			<td style="width: 110px;" class="TableData"><select id="type" name="type" class="BigSelect" style="height:25px;">
			...
			<td class="TableData">
			从&nbsp;<input type="text" id='startTime' name='startTime' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'endTime\')}'})" class="Wdate BigInput" style="width:100px;height:25px;" />&nbsp;
			到<input type="text" id='endTime' name='endTime' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startTime\')}'})" class="Wdate BigInput" style="width:100px;height:25px;"/>
