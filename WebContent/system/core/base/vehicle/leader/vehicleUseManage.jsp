...
function doInit(){
    var postMeetRoom = getAllVehicle();
    var liStr = "";
   
	if(postMeetRoom.length > 0){
		  var tableStr = "<table class='TableBlock_page' width='100%' align='center'><tbody id='listTbody'>";
		  tableStr = tableStr + "<tr class='TableHeader' id='tbl_header'>"
	     	 + "<td width='250px' align='center'  >车辆名称/时间</td>";
		  for(var i = 0;i<24 ; i++){
		...
		for(var j = 0;j<postMeetRoom.length ; j++){
	    	var meetRoom = postMeetRoom[j];
	    	  tableStr = tableStr + "<tr class=''>"
		     	 + "<td width='250px' align='left'  class='TableData'>" + meetRoom.vModel + "</td>";
	    	 // for(var i = 0;i<24 ; i++){
 ...
 * 获取所有会议室
 */
function getAllMeet(startTimeStr ,today){
	//清空数据
	...
	var url =   "<%=contextPath %>/vehicleUsageManage/getAllVehicleByTime.action";
	var para = {vuStartStr:startTimeStr,vuEndStr:startTimeStr};
		...
		if(prcs.length> 0){
			...
			var week_width = 0;
			 ...
			 for(var i = 0 ; i<prcs.length ; i++){
				 var prc = prcs[i];
				 var id = prc.sid;
				 var meetName = prc.vehicleName;
				 var startTimeStr = prc.vuStartStr;
				 var endTimeStr = prc.vuEndStr;
				 var meetRoomId = prc.vehicleId;
...
<body  topmargin="5" onload="doOnload();" style="padding-left: 10px;padding-right: 10px;padding-top:20px">
	...
	<div id="toolbar" style="min-width:800px;">
		<div class="moduleHeader">
			...
			<div class="btn-group pull-left">
	            ...
	            <span class="fc-header-space"></span><span class="fc-button fc-button-today fc-state-default fc-corner-left fc-corner-right fc-state-disabled" unselectable="on" onclick="getAllMeet('<%=startTimeStr %>','1')">今天</span>
			</div>
