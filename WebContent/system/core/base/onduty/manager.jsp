...
function findDeptAll(){
	var url = "<%=contextPath%>/teePbOnDutyController/dutyDeptAll.action";
	var json=tools.requestJsonRs(url,null);
	var data=json.rtData;
	if(data!=null && data.length>0){
		for(var i=0;i<data.length;i++){
...
function renderCalendar(){
	...
	//获取当前月的所有值班信息
	var url = "<%=contextPath%>/teePbOnDutyController/findDutyByDate.action";
	var para = {year : year,month : month,totalOfMonthDays : totalOfMonthDays,deptAll:deptAll,dutyTypeName:dutyTypeName};
			...
			if(data!=null){
				for(var m=0;m<data.length;m++){
					if(data[m].creTime==dataStr){
		    			...
		    			var time=year+"-"+month+"-"+(i+1);
		    			var menuData = [];
					    	menuData.push({name:"查看",action:function(sid){
					    		view(sid);
							} ,extData:[allUuid]});
							menuData.push({name:"编辑",action:function(sid,time){
								update(sid,time);
							} ,extData:[allUuid,time]});
...
<body onload="doInit()">
	<div id="toolbar" class = "topbar clearfix" style="border-bottom: 1px solid #74c5ff!important;">
	   <div class="fl" style="position:static;">
		  ...
		  <span class="title">值班表管理</span>
	   </div>
	   <div class = "right fr clearfix" style="margin-right:8px;">
	   <input type="button" style="" id="quanXuan" value="全选" class="btn-win-white fl" onclick="quanXuan();"/>&nbsp;&nbsp;
		<input type="button" style="" id="fanXuan" value="反选" class="btn-win-white fl" onclick="fanXuan();"/>&nbsp;&nbsp;
		<input type="button" class="btn-win-white fl" id="batch" value="设置" onclick="batch();">&nbsp;&nbsp;
		<input type="button" style="" id="batchDelete2" value="导入" class="btn-win-white fl" onclick="toImport();"/>
		<input type="button" style="" id="batchDelete" value="删除" class="btn-del-red" onclick="batchDelete();"/>&nbsp;
			<!-- 年份 -->
			...
			<!-- 值班类型 -->
			<select id="dutyTypeName" class="BigSelect" onchange="renderCalendar()">
				<option value="0">全部值班</option>
			</select>
			<select id="deptAll" class="BigSelect" onchange="renderCalendar();">
