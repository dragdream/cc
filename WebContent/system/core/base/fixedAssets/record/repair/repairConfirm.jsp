...
// 	getDeptParent();
	datagrid = $('#datagrid').datagrid({
		url:contextPath+"/teeFixedAssetsRecordController/getRecordDatagrid.action?optType=5",
		pagination:true,
		singleSelect:false,
		...
		columns:[[
			...
			{field:'repairUser',title:'维修负责人',width:200},
			{field:'repairCost',title:'维修费用',width:200},
			{field:'userName',title:'申请人',width:200},
			{field:'deptName',title:'申请人部门',width:200},
			{field:'optDateDesc',title:'申请日期',width:200},
			{field:'_opt',title:'操作',width:200,formatter:function(data,rowData){
				return "<a href='javascript:void(0)' onclick='repairConfirm("+rowData.sid+")'>维修确认</a>";
			}}
		]]
	});
}
 ...
 */
function apply(sid){
	 $.MsgBox.Confirm ("提示","是否发起资产报修申请？",function(){
		 createNewWork(43);
		});
}

function repairConfirm(recordId){
	var json = tools.requestJsonRs(contextPath+"/teeFixedAssetsRecordController/repairConfirm.action",{recordId:recordId});
 ...
 * 报修
 */
function applyBack(sid){
	var title = "固定资产报修返库";
	var  url = contextPath + "/system/core/base/fixedAssets/record/repair/repairBack.jsp?fixedAssetsId=" + sid;
...
<body onload="doInit()" style="overflow: hidden; font-size: 12px; padding-left: 10px; padding-right: 10px;">
	...
	<div id="toolbar" class="clearfix" style="padding-top: 10px;">
		<div class="fl" style="position: static;">
			<img id="img1" class='title_img' src="<%=contextPath %>/common/zt_webframe/imgs/xzbg/gdzc/icon_wxqr.png">&nbsp;&nbsp;
				<span class="title">维修确认</span>
		</div>
			...
			<div style="padding: 0px;" id="searchDiv">
				<table>
					<tr>
						...
						<td nowrap class="TableData" style="width: 180px;"><input type="text" id="assetCode" name='assetCode' style="height: 25px;font-family: MicroSoft YaHei;" />
						</td>
						<td class="TableData TableBG">资产名称：</td>
						<td nowrap class="TableData" style="width: 180px;">
						<input class="BigInput" type='text' id='assetName' name='assetName'
							style="height: 25px;font-family: MicroSoft YaHei;" /></td>
