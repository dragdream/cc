...
function doInit(){
	//alert(paraValue);
	var url = "<%=contextPath %>/hrCode/getSysParaByParent.action?parentId=" + parentId;
...
function deleteById(id){
	if(confirm("确认要删除此编码？")){
		var url = "<%=contextPath %>/hrCode/delById.action";
...
function toAddOrUpdate(id){
	var url = contextPath + "/system/core/base/hr/setting/code/addupdatechild.jsp?sid=" + id + "&parentId=" + parentId;
	var title = "新增子代码";
	if(id>0){
		title = "编辑子代码";
	}
...
<body onload="doInit()">
   <table border="0" width="95%" cellspacing="0" cellpadding="3" class="small" align="center">
    <tr>
      <td class="Big"><img src="<%=imgPath%>/sys_config.gif" align="absmiddle"><span class="big3">HR子编码管理</span></td>
    </tr>
  </table><br>
