...
function createUserPrivTable(fileId){
  $("#bodyDiv").html("");
  var table = "<table id='userPrivTab' class='TableList' width='98%' align='center'>" 
      ...
      + "<td nowrap  ><input type='checkbox' id='userEditPriv' onclick='getSelectFunc(this,1)' >&nbsp;<label for='userEditPriv' >编辑权限</label> </td>"
      + "<td nowrap  ><input type='checkbox' id='userDownPriv' onclick='getSelectFunc(this,2)' >&nbsp;<label for='userDownPriv' >下载权限 </label> </td>"
      + "<td nowrap  style='display:none;' ><input type='checkbox' id='userUploadPriv'onclick='getSelectFunc(this,3)'>&nbsp;<label for='userUploadPriv'>上传权限</label> </td>"
      + "<td nowrap  style='display:none;' ><input type='checkbox' id='userDeletePriv'onclick='getSelectFunc(this,4)'>&nbsp;<label for='userDeletePriv'>删除权限 </label> </td>"
      + "<td nowrap  style='display:none;' ><input type='checkbox' id='userManagePriv'onclick='getSelectFunc(this,5)'>&nbsp;<label for='userManagePriv'>管理权限 </label> </td>"
      + "<td nowrap  >操作 </td>"
      + "</tr></table>";
...
function showUserPriv(){
  var html = "<div style='padding:10px;' style='background-color:#f2f2f2'>" 
    ...
    +          "<span class='addSpan'>"
    +          "<img src='/common/zt_webframe/imgs/grbg/gryj/icon_select.png' onClick=\"selectUser(['personIds','personIdsName'],'1')\"/>"
    +          "&nbsp;&nbsp;<img src='/common/zt_webframe/imgs/grbg/gryj/icon_cancel.png' onClick=\"clearData('personIds','personIdsName')\"/>"
    +          "</span>"
    +       "</td>"
    +      "</tr>"
    +  "</table>"
 ...
 window.parent.bsWindow(html ,"人员权限",{width:"500",height:"130",
	    buttons:[
 ...
 /*$.jBox.open("html:"+ html ,"人员权限",500,200,{ submit:function(v,h,f){
    ...
    if(!f.personIds){
      $.jBox.tip("请选择人员！", "error", {timeout: 1500}); 
      return;
    }
    //return ;
    //点击保存选择的人员
      ...
      if(personIds && personNameStr){
        if(!isContainPerson (personObjArray,personIds)){
          ...
          var curTr = "<tr>"
            ...
            +   "<td style='text-indent:10px'>" + trCount + "</td>"
            +   "<td>"+ personNameStr + "</td>"
            ...
            +   "<td style='display:none;' ><input type='checkbox'  name='personCheckbox_3_" + personIds + "' value='8'></td>"
            +   "<td style='display:none;' ><input type='checkbox'  name='personCheckbox_4_" + personIds + "' value='16'></td>"
            +   "<td style='display:none;' ><input type='checkbox'  name='personCheckbox_5_" + personIds + "' value='32'></td>"
    ...
    for(var i=0;i<arrayObj.length;i++){
      ...
      for(var k=0;k<6;k++){
        ...
        $("input[name=personCheckbox_" + k + "_" + personId + "]").each(function(id,obj){
          if(obj.checked){
            userPriv += parseInt(obj.value,10);
          }
          //alert(id + ">>" +obj.name + ">>" + obj.value + ">>" + obj.checked);
        });
      }
      //alert("name=personCheckbox_" +i + "_" + roleId + " userPriv>>" + userPriv );
...
function submitPersonPriv(){
  ...
  //转换成组织对象
  var userPrivObj = eval('('+ userPrivJson + ')');
  if(userPrivObj.length>0){
    var para = {userPrivJson:userPrivJson,fileId:fileId};
    var url = contextPath + "/fileUserPriv/addFileUserPriv.action";
    var jsonRs = tools.requestJsonRs(url,para);
    if(jsonRs.rtState){
    	$.MsgBox.Alert_auto("权限设置成功！");
        setTimeout(function(){
          var nodeObj = {id:fileId,iconSkin:"file_folder_share"};
          parent.parent.frames["file_tree"].updateZTreeNode(nodeObj);  
    ...
    var nodeObj = {id:fileId,iconSkin:"file_folder"};
    parent.parent.frames["file_tree"].updateZTreeNode(nodeObj); 
    $.MsgBox.Alert_auto("请添加用户！");
  }
}
...
function doInitUserPriv(sid){
  ...
  var url = contextPath + "/fileUserPriv/getFileNetdiskPersonPriv.action";
  var jsonRs = tools.requestJsonRs(url,para);
