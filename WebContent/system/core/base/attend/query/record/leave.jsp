...
var datagrid;
function queryLeave(){
<%-- 	var url =   "<%=contextPath %>/attendLeaveManage/getLeave.action?status=9&userId=<%=userId%>"; --%>
	datagrid = $('#datagrid').datagrid({
		url:contextPath+'/attendLeaveManage/getLeaveEasyui.action?status=9&userId=<%=userId%>',
		...
		if(prcs.length > 0){
			var tableStr = "<table class='TableBlock' width='98%' align='center'><tbody id='listTbody'>";
		    	 tableStr = tableStr + "<tr class='TableHeader'>"
			      	...
			      	//木有了 +"<td nowrap  width='80' align='center'>操作</td>"
			         +"</tr>";
			for(var i = 0;i<prcs.length ; i++){
				var prc = prcs[i];
				var id = prc.sid;
				var  fontStr = "";
				var leaveDesc = prc.leaveDesc.length>20?prc.leaveDesc.substring(0,20)+"...":prc.leaveDesc;
				tableStr = tableStr +"<tr class='TableData'>"
				     	 ...
				     	 +"<td nowrap align='center'><font color='" + fontStr + "'>" + prc.leaveTypeDesc + "</font></td>"
				      	 +"<td nowrap align='center'><font color='" + fontStr + "'><a href='javascript:void(0);' onclick='attendLeaveInfo(" + id + ")' title='"+prc.leaveDesc+"'>" +leaveDesc  + "</a></font></td>"
			...
			$("#listDiv").append(tableStr);	
		 	...
		 	messageMsg("没有相关请假历史记录", "listDiv" ,'' ,380);
		}
	}else{
		alert(jsonObj.rtMsg);
	} */
