...
function doInit(){
	datagrid = $('#datagrid').datagrid({
		url:contextPath + "/TeeDutyComplaintController/datagrid.action?status=0",
		...
		columns:[[
			...
			{field:'opt_',title:'操作',width:200,formatter:function(value,rowData,rowIndex){
			   ...
			   opt+="<a href=\"#\" onclick=\"approve(1,"+rowData.sid+")\">批准</a>&nbsp;&nbsp;&nbsp;&nbsp;";
			   opt+="<a href=\"#\" onclick=\"approve(2,"+rowData.sid+")\">不批准</a>";
			   return opt;
			}},
...
function approve(status,sid){
  ...
  $.MsgBox.Confirm ("提示", mess, function(){
	  var url = contextPath + "/TeeDutyComplaintController/approve.action";
		var para = {status:status,sid:sid};
