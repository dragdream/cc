...
function queryEvection(){
	datagrid = $('#datagrid').datagrid({
		url:contextPath+"/attendEvectionManage/getLeaderEvection.action",
		...
		columns:[[
			...
			{field:'ope_',title:'操作',width:160,formatter:function(value,rowData,rowIndex){
				var opts =  "<a href='javascript:void(0);'  onclick='approv(\"" + rowData.sid + "\",0," + rowData.userId + ");'>批准 </a>"
		      	 +"&nbsp;&nbsp;<a href='javascript:void(0);' class='modal-menu-test' onclick='$(this).modal();approv(\"" + rowData.sid + "\",1 , " + rowData.userId + ");' >不批准</a>";
				return opts;
			}},
		]]});
}
 ...
 *查询已审批记录
 */
function queryApprovEvection(){
	datagrid = $('#datagrid').datagrid({
		url:contextPath+"/attendEvectionManage/getLeaderApprovEvection.action",
		view:window.EASYUI_DATAGRID_NONE_DATA_TIP,
		pagination:true,
		singleSelect:true,
		toolbar:'#toolbar',//工具条对象
...
function approv(sid , allow , userId){
	if(allow == 0){//批准
		$.MsgBox.Confirm ("提示", "确定批准此出差申请吗？", function(){
			var url =   "<%=contextPath %>/attendEvectionManage/approve.action";
...
<div id="toolbar" class="clearfix">
	<div class="right fr setHeight option_buttons" >
	   <button type="button" value="待审批出差" class="btn-win-white fl btn_active" onclick="queryEvection();">待审批出差</button>
	   &nbsp;
	   <button type="button" value="已审批出差" class="btn-win-white fl" onclick="queryApprovEvection();">已审批出差</button> 
	</div>
