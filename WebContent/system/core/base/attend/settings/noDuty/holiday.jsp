...
function doInit(){
	datagrid = $('#datagrid').datagrid({
		url:contextPath+"/TeeAttendConfigController/datagridHoliday.action",
		...
		columns:[[
			...
			{field:'holidayName',title:'节日名称',width:100},
			...
			{field:'2',title:'操作',formatter:function(e,rowData,index){
					return "<a href='javascript:void();' onclick='edit("+rowData.sid+")'>编辑"+
					"</a>&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick='del("+index+")'>删除</a>"+
					"&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick=workDayList('"+rowData.sid+ "','"+ rowData.holidayName + "')>补假管理</a>&nbsp;&nbsp;&nbsp;";
			}}
		]]
	});
...
function edit(sid){
	...
	/* bsWindow(contextPath+"/system/core/base/attend/settings/noDuty/holiday_modify.jsp?sid="+sid,"修改免签节日",{width:"500",height:"150",submit:function(v,h){
	...
	var url = contextPath+"/system/core/base/attend/settings/noDuty/holiday_modify.jsp?sid="+sid;
	var title = "修改免签节日";
...
function add(){
	/* bsWindow(contextPath+"/system/core/base/attend/settings/noDuty/holiday_add.jsp","添加免签节日",{width:"500",height:"150",submit:function(v,h){
		var cw = h[0].contentWindow;
		if(cw.commit()){
			datagrid.datagrid("unselectAll");
			datagrid.datagrid("reload");
	...
	var url = contextPath+"/system/core/base/attend/settings/noDuty/holiday_add.jsp";
	var title = "添加免签节日";
	bsWindow(url,title,{width:"500",height:"150",buttons:
		 [{name:"确定",classStyle:"btn-alert-blue"},{name:"关闭",classStyle:"btn-alert-gray"}]
	,submit:function(v,h,f,d){
...
function workDayList(sid,name){
	var url = "<%=contextPath%>/system/core/base/attend/settings/noDuty/workDayList.jsp?sid=" + sid + "&name=" + encodeURIComponent(name);
	location.href = url;
}
</script>
...
<body onload="doInit()" style="font-size:12px;padding-left: 10px;padding-right: 10px">
	...
	<div id="toolbar" class = "topbar clearfix">
		<!-- <div class="moduleHeader">
			<b><i class="glyphicon glyphicon-sound-stereo"></i>&nbsp;免签节日管理</b>
		</div> -->
