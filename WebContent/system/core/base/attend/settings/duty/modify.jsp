...
function commit(){
	if($("#form1").valid() && checkForm()){
		...
		var url = contextPath+"/TeeAttendConfigController/updateConfig.action";
...
function doInit(){
	if(sid!="" && sid!=null && sid!="null"){
		...
		if(json.rtState){
			...
			var addrJson=json.rtData.addrJson;
			renderAddr(addrJson);
		}else{
...
//渲染签到地点
var totalId=0;
function renderAddr(addrJson){
	var jsonMap=tools.string2JsonObj(addrJson);
	 var html=[];
	for(var n=0;n<jsonMap.length;n++){
		  html.push("<tr class='TableData addressTr' >"+
				    ...
				    "</td>"+
				    "<td>"+
					"<input type=\"text\"  style=\"height:21px;width: 300px;\" id='centerPositionDesc"+totalId+"'  name='centerPositionDesc"+totalId+"'  value='"+jsonMap[n].centerPositionDesc+"'     />"+
					"<input type=\"hidden\"  style=\"height:21px;\"   id='centerPosition"+totalId+"'  name='centerPosition"+totalId+"'  value='"+jsonMap[n].centerPosition+"'    />"+
				    ...
				    "</td>"+
				    "<td>"+
					"<input type=\"text\"  style=\"height:21px\"  id='radius"+totalId+"'  name='radius"+totalId+"' value='"+jsonMap[n].radius+"' /> （米）"+		
				    "</td>"
			        +"</tr>");
...
<form id="form1" name="form1">
	<table  style="width:100%;font-size:12px" class='TableBlock' id="tb1">
		<tr class='TableData'>
			...
			<td>
				<input type="text"   id="dutyName" name="dutyName" required="true" class="easyui-validatebox BigInput"/>
