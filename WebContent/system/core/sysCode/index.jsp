...
function changePage(sel , sid){
	if(sel==1){
		$("#frame0").attr("src",contextPath+"/system/core/sysCode/blank.jsp");//外出
	}else if(sel==2){
		$("#frame0").attr("src",contextPath +"/system/core/sysCode/addupdate.jsp?sid=" + sid);//请假
	}
}
function getCrmList(){
	//添加例子一
...
function toChild(sid,paraValue,paraName){
	$("#frame0").attr("src", "<%=contextPath%>/system/core/sysCode/childPara.jsp?paraValue=" + encodeURIComponent(paraValue) + "&sid=" + sid + "&paraName=" + encodeURIComponent(paraName));
}
</script>
	...
	<div layout="west" width="430" style="overflow:auto;">
		...
		<div id="group" class="list-group">
		    <table class="TableList" width="80%" align="center">
		        <tr>
		            <td colspan="3"> <a href="javascript:changePage(2,'');">新增系统主编码</a></td>
		        </tr>
