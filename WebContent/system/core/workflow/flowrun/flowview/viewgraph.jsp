...
<!-- DEP -->
		...
		<!-- /JS -->
		<script type="text/javascript" src="<%=contextPath%>/common/js/flowdesigner/flowdesignerForView.js"></script>
...
window.onload=function(){
	...
	for(var i=0;i<prcsList.length;i++){
		...
		var title = "<div style=\"max-width:420px\"><p>"+prcsId+":"+prcsName+"<p>";
		var array = dataList;
		var hasBegin = false;//是否是开始节点
		var firstNode = null;
		//循环查找对应走过的步骤，并渲染
		for(var j=0;j<array.length;j++){
			var prcsInfo = array[j];
			if(prcsInfo.flowPrcsId!=sid){
				continue;
			}
			firstNode = prcsInfo;
...
function getCurrentFlowProcess(){
	...
	//渲染并绘制步骤信息
	if(json.rtState){
		dataList = json.rtData.prcsList;
		isEnd = json.rtData.isEnd;
		curFlowPrcs = dataList[dataList.length-1].flowPrcsId;
	}
}
</script>
