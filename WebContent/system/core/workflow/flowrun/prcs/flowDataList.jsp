...
function doInit(){
	...
	var opts = [{field:'sid',checkbox:true,title:'ID',width:200},{
		title : '流水号',
		field : 'RUN_ID',
		ext:'@流水号',
		sortable:true,
...
function selectIt(){
	...
	if(mappings!=null&&mappings.length>0){
		for(var i=0;i<mappings.length;i++){
			var data=selected[maps[mappings[i].title1]];
			var item = xparent.findTargetByTitle(mappings[i].title2);
			if(item.length!=0){
				item.attr("value",data);
			}
		}
	}
	xparent.commonSelect(selected);
	CloseWindow();
