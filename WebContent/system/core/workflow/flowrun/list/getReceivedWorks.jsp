...
%>
	...
	function doDelegate(rowIndex){
		...
		bsWindow(url,"委托",{width:"600",height:"200",buttons:
		[
...
// 		$(obj).removeAttr("href").html("<span title='刷新可重新下载' style='color:green'>导出中请等待</span>")[0].onclick=function(){return false;};
	...
	function query(){
		...
		var opts = [{title:'流水号',
				...
				{field:'RUN_NAME',
					...
					formatter:function(a,data,c){
						...
						render = render+"<a title='"+data.runName+"' href='javascript:void(0)' onclick=\"doHandler('"+c+"')\" >"+data.runName+"</a>";
						if(data.backFlag==1){
		...
		datagrid = $('#datagrid').datagrid({
			url:'<%=contextPath%>/workflow/getReceivedWorks.action?flowSortId=<%=flowSortId%>',
...
// 			if(confirm("是否确认导出所有查询结果？")){
	        $.MsgBox.Confirm ("提示", "是否确认导出所有查询结果？", function(){
				...
				params["type"]="1";//用来区分
				params["runIds"]="0";
				$("#frame0").attr("src",contextPath+"/workflow/exportExcel.action?params="+encodeURIComponent(tools.jsonObj2String(params)));
	        });
// 			}else{
// 				return;
// 			}
...
function selectFlowType(){
	...
	var url=contextPath+"/workflow/getPrivFlowTypeIds.action?type=1";
...
<!-- 				<td class="TableData"><input type="text" name="runId" class="BigInput" style="height:20px;width: 80px"/></td> -->
