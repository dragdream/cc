...
<script>
	...
	function getBasicFormItem(){
		var url = contextPath+"/teeHtmlPrintTemplateController/getBasicFormItemsByFlowType.action";
		var json = tools.requestJsonRs(url,{flowTypeId:flowTypeId});
		if(json.rtState){
			 var fieldList=json.rtData;
			 for(var i=0;i<fieldList.length;i++){
				 var btn = $("<button style='width:100%' class='btn btn-default' onclick='insertField(this)' id="+fieldList[i].name+"  name='"+fieldList[i].title+"'  >"+fieldList[i].title+"</button>");
	...
	function getFormInfo(){
		...
		if(json.rtState){
			 var form=json.rtData.tplContent;	
			 return form;
		}else{
			return null;
		}
	...
	function insertField(obj){
		editor.insertHtml("<img src='"+contextPath+"/bisFormController/ctrlShow.action?fn="+encodeURI(obj.name)+"' id='"+obj.id+"' name='"+obj.id+"' value='"+obj.name+"' class='DATAFIELD' />");
	}
...
// 		}
		...
		var content = editor.getData();
		var url = contextPath+"/teeHtmlPrintTemplateController/updateTplContent.action";
		var json=tools.requestJsonRs(url,{sid:sid,tplContent:content});
		if(json.rtState){
			alert("保存成功！");
		}
	}
