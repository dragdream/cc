...
<body>
	<header class="mui-bar mui-bar-nav">
		...
		<h1 class="mui-title">商机（<%=typeDesc %>）</h1>
	...
	function pulldownRefresh() {
		...
		var url="";
		if(type!=0||type!=null){
			url = contextPath+'/crmChancesController/datagrid.action?type='+type;
		}
		...
		mui.ajax(url,{
			...
			success:function(json){
				...
				for(var i=0;i<json.rows.length;i++){
					...
					render.push("<p style='font-size:12px'>所属客户："+item.customerName+"<span style='padding-left:15px;'>"+item.chanceStatusDesc+"</span></p>");
	...
	function detail(){
		...
		window.location = contextPath+"/system/mobile/phone/crm/chances/chancesInfo.jsp?sid="+uuid+"&customerName="+customerName+"&type="+type;
	}
	...
	mui.ready(function() {
		...
		addBtn.addEventListener("tap",function(){
			window.location.href = contextPath+"/system/mobile/phone/crm/chances/addOrUpdate.jsp?type="+type;
		});//新建
