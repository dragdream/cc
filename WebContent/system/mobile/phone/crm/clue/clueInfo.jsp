...
function doInit(){
	...
	//返回
 	backBtn.addEventListener("tap",function(){
		window.location = contextPath+'/system/mobile/phone/crm/clue/index.jsp?type='+type;
	});
	
}

//更换负责人
function changeCharge(sid){
	window.location = contextPath+'/system/mobile/phone/crm/clue/changeCharge.jsp?sid='+sid+'&managerPerId='+managerPerId+'&managerPerName='+managerPerName+'&type='+type;
}
...
function renderOptBtns(data){
	...
	$("#topPopover1").empty();
 	if(managerPerId==loginPersonId){
 		if(data == "待处理"||data=="已处理"){
			$("#topTop").show();
		   str ='<ul class="mui-table-view">'+
		     '<li class="mui-table-view-cell" onclick="followUp('+sid+');">跟进中</li>'+
		     '<li class="mui-table-view-cell" onclick="transClue('+sid+');">转换线索</li>'+
		   	 '<li class="mui-table-view-cell" onclick="invalid('+sid+');">无效</li>'+
    		 '<li class="mui-table-view-cell" onclick="editClue('+sid+');">编辑</li>'+
	  		 '<li class="mui-table-view-cell" onclick="changeCharge('+sid+');">更换负责人</li>'+
	...
	mui.ajax(url,{
		...
		success:function(json){
			...
			if(json.rtState){
				...
				clueStatus = data.clueStatusDesc;
...
//跟进中
function followUp(sid){
	window.location = 'followUp.jsp?sid='+sid+'&type='+type;
}

//转换线索
function transClue(sid){
	window.location = 'transfromClue.jsp?sid='+sid+'&type='+type;
}

//无效
function invalid(sid){
	window.location = 'invalid.jsp?sid='+sid+'&type='+type;
}

//编辑
function editClue(sid){
	window.location = 'addOrUpdate.jsp?sid='+sid+'&type='+type;
}
