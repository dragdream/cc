...
<%
	...
	int invoiceStatus=TeeStringUtil.getInteger(request.getParameter("invoiceStatus"),1);
%>
<!DOCTYPE HTML>
<html>
<head>
...
//保存/提交
function save(){
	var url=contextPath+"/TeeCrmInvoiceController/reject.action";
	var param=formToJson("#form1");
	param['sid']=sid;
	param['invoiceStatus']=3;
	param["isPhone"]=1;
	mui.ajax(url,{
	type:"post",
	dataType:"html",
	...
	success:function(json){
		json = eval("("+json+")");
		if(json.rtState){
			window.location.href=contextPath+"/system/mobile/phone/crm/invoice/indexApprove.jsp?invoiceStatus="+invoiceStatus;
		}
			}
		});
}
...
mui.ready(function() {
	...
	backBtn.addEventListener("tap",function(){
		if(sid>0){
			...
			window.location.href=contextPath+"/system/mobile/phone/crm/invoice/indexApprove.jsp?type="+type;
		}
	});
