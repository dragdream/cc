...
<%
    ...
    //判断是不是开票审批页面跳过来的
    int isApprove=TeeStringUtil.getInteger(request.getParameter("isApprove"), 0);
    int invoiceStatus=TeeStringUtil.getInteger(request.getParameter("invoiceStatus"), 0);
%>
<!DOCTYPE HTML>
<html>
<head>
...
<form id="form1" name="form1">
	<div class="mui-input-group">
		...
		<div class="mui-input-row">
			<label>开票编号</label>
			<input type="text" placeholder="请填写开票编号" name="invoiceNo" id="invoiceNo">
		</div>
		<div class="mui-input-row">
			<label>所属客户</label>
		...
		<div class="mui-input-row">
			...
			<select id="orderNo"  name="orderNo" style="margin-left:0px;width:65%;font-size: 14px;">
	       		<option value="0">选择销售订单</option>
	        </select>
		</div>
		<div class="mui-input-row">
			<label>开票日期</label>
			<input type="text" placeholder="请选择开票日期" name="invoiceTimeDesc" id="invoiceTimeDesc">
		</div>
		<div class="mui-input-row">
			<label>开票金额（元）</label>
			<input type="text" placeholder="请填写开票金额" name="invoiceAmount"  id="invoiceAmount"/>
		</div>
		<div class="mui-input-row">
			<label>开票类型</label>
			<select id="invoiceType" placeholder="开票类型"  name="invoiceType" style="margin-left:0px;width:65%;font-size: 14px;">
	        </select>
		</div>
		<div class="mui-input-row">
			<label>备注</label>
		...
		<div class="mui-input-row">
			<label>发票号码</label>
			<input type="text" placeholder="请填写发票号码" name="invoiceNumber"  id="invoiceNumber"/>
		</div>
		<div class="mui-input-row">
			<label>开票财务</label>
			<select id="invoiceFinancialId"  name="invoiceFinancialId" style="margin-left:0px;width:65%;font-size: 14px;">
	        </select>
		</div>
		<div class="mui-input-row">
			<label>附件</label>
	...
	<div class="mui-input-group">
		...
		<div class="mui-input-row">
			<label>开票抬头</label>
			<input type="text" placeholder="请填写开票抬头" name="invoiceHeader"  id="invoiceHeader"/>
		</div>
		<div class="mui-input-row">
			<label>纳税人识别号</label>
			<input type="text" placeholder="请填写纳税人识别号" name="nsrNumber"  id="nsrNumber"/>
		</div>
		<div class="mui-input-row">
			<label>开户行账号</label>
			<input type="text" placeholder="请填写开户行账号" name="khhNumber"  id="khhNumber"/>
		</div>
		<div class="mui-input-row">
			<label>开户行地址</label>
			<input type="text" placeholder="请填写开户行地址" name="khhAddress"  id="khhAddress"/>
		</div>
		<div class="mui-input-row">
			<label>开户行名称</label>
			<input type="text" placeholder="请填写开户行名称" name="khhName"  id="khhName"/>
		</div>
		<div class="mui-input-row">
			<label>电话</label>
			<input type="text" placeholder="请填写电话" name="telephone"  id="telephone"/>
		</div>
	</div>
      ...
      <div class="mui-input-group">
		...
		<div class="mui-input-row">
			<label>联系人</label>
			<input type="text" placeholder="请填写联系人" name="contacts"  id="contacts"/>
		</div>
		<div class="mui-input-row">
			<label>联系方式</label>
			<input type="text" placeholder="请填写联系方式" name="contactNumber"  id="contactNumber"/>
		</div>
		<div class="mui-input-row">
			<label>寄送地址</label>
			<input type="text" placeholder="请填写寄送地址" name="sendAddress"  id="sendAddress"/>
		</div>
	</div>
</form>	
</div>
	...
	if(check()){
		...
		UploadPhoto(function(files){
			...
			success:function(json){
				...
				if(json.rtState){
					if(sid>0){
						if(isApprove==1){//代表是开票审批页面跳过来的
							window.location.href=contextPath+"/system/mobile/phone/crm/invoice/approveInvoiceInfo.jsp?sid="+sid+"&customerName="+cusName+"&invoiceStatus="+invoiceStatus;
						}else{
							window.location.href=contextPath+"/system/mobile/phone/crm/invoice/invoiceInfo.jsp?sid="+sid+"&customerName="+cusName+"&type="+type;
						}
