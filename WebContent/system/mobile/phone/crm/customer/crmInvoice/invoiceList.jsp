...
<body>
	...
	function pulldownRefresh() {
		...
		var url="";
		if(type!=0||type!=null){
			url = contextPath+'/TeeCrmInvoiceController/datagrid.action?cusId='+cusId;//开票申请列表
		}
		...
		mui.ajax(url,{
			...
			success:function(json){
				...
				for(var i=0;i<json.rows.length;i++){
					...
					render.push("<p style='font-size:12px'>"+item.managePersonName+"<span style='padding-left:15px;'>"+item.invoiceStatusDesc+"</span></p>");
...
function detail(){
	...
	window.location = contextPath+"/system/mobile/phone/crm/customer/crmInvoice/invoiceDetail.jsp?sid="+sid+"&customerId="+cusId+"&customerName="+customerName+"&type="+type;
}
	...
	mui.ready(function() {
		...
		addBtn.addEventListener("tap",function(){
			window.location.href = contextPath+"/system/mobile/phone/crm/customer/crmInvioce/addOrUpdate.jsp?customerId="+cusId+"&customerName="+customerName+"&type="+type;
		});//新建
