...
function doInit(){
	...
	backBtn.addEventListener("tap",function(){
		window.location ="detail.jsp?taskPubRecordId="+taskPubRecordId+"&reportTime="+reportTime+"&taskTemplateName="+taskTemplateName+"&taskTemplateId="+taskTemplateId;
	});
}
			...
			if(json.rtState){
				...
				if(dataList!=null&&dataList.length>0){
					...
					for(var i=0;i<dataList.length;i++){
						...
						render.push("<label><div>"+dataList[i].fieldValue+"</div></label>");
						render.push("</div>");
...
//历史信息  查看详情
function detail(sid){
	window.location.href="info.jsp?taskPubRecordItemId="+sid+"&taskTemplateName="+taskTemplateName+"&taskTemplateId="+taskTemplateId+"&createUserId="+createUserId+"&taskPubRecordId="+taskPubRecordId+"&reportTime="+reportTime;
}
</script>
<body onload="doInit();">
