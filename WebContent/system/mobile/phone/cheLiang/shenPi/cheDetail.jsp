...
<header id="header" class="mui-bar mui-bar-nav">
	<span class="mui-icon mui-icon-back" onclick="window.location.href='shenPindex.jsp?status=<%=status %>'"></span>
	<a class="mui-action-menu mui-icon mui-icon-more mui-pull-right" id="topPopoverXs" style="display: none;" href="#topPopover"></a>
...
<!--右上角弹出菜单-->
	<div id="topPopover" class="mui-popover">
		<ul class="mui-table-view">
		   <li class="mui-table-view-cell" id="piZhun">批准</li>
		   <li class="mui-table-view-cell" id="notPiZhun">不批准</li>
		  </ul>
	</div>
...
function doInit(){
	...
	if(status==0){//批准和不批准
	 ...
	 $("body").on("tap","#piZhun",function(){//批准
		 approvalFunc(1);
	});
	 $("body").on("tap","#notPiZhun",function(){//不批准
		 approvalFunc(3);
...
function approvalFunc(state){
	  ...
	  if(window.confirm(remaindStr)){
	    	$.ajax({
				  type: 'POST',
				  url: contextPath + "/vehicleUsageManage/approval.action",
				  data: {sid:sid,status:state},
				  timeout: 10000,
				  async:false,
				  success: function(json){
					 alert(mess);
					 window.location.href="shenPindex.jsp?status="+status;
				  },
