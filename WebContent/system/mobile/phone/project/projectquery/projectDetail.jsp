...
function doInit(){
	...
	a2.addEventListener("tap",function(){
		window.location = 'taskList.jsp?projectId=<%=uuid%>';
	});
	//返回
...
//		 		}
				...
				//项目状态  立项中1       审批中2      办理中3        挂起中4       已办结5     已拒绝6
				status=data.status;
				var statusDesc="";
				if(status==1){
					statusDesc="立项中";
...
function  createTask(uuid){
	var url=contextPath+"/system/mobile/phone/project/projectquery/addTask.jsp?projectId="+uuid;
...
function  change(uuid){
	var url=contextPath+"/system/mobile/phone/project/projectquery/addOrUpdate.jsp?uuid="+uuid;
	window.location.href=url;
}
