...
<%
	if("".equals(typeId)){
		...
		out.print(TeeSysCodeManager.getChildSysCodeNameCodeNo("NOTIFY_TYPE", typeId));
	}
%>
</title>
<script>
...
<body>
    <header class="mui-bar mui-bar-nav">
		<h1 class="mui-title"><%=stateDesc %>公告</h1>
	...
	<div id="topPopover" class="mui-popover">
		<ul class="mui-table-view">
		    <li class="mui-table-view-cell" onclick="window.location = 'index.jsp?state=0';">未读公告</li>
		    <li class="mui-table-view-cell" onclick="window.location = 'index.jsp?state=1';">已读公告</li>
		  </ul>
	</div>
	<script>
	    var state=<%=state%>;
		...
		function pulldownRefresh() {
			...
			if(state==0){
				...
				url=contextPath+"/teeNotifyController/getReadedNotify.action";
			}
			...
			mui.ajax(url,{
				...
				success:function(json){
					...
					for(var i=0;i<json.rows.length;i++){
						...
						render.push("<p style='font-size:12px'>"+item.fromPersonName+"&nbsp;&nbsp;"+item.sendTimeDesc+"</p>");
						render.push("</div>");
						render.push("</li>");
					}
		...
		function detail(){
			...
			if(userAgent.indexOf("DingTalk")==-1 && userAgent.indexOf("MicroMessenger")==-1){//原生
				OpenWindow('',contextPath+"/system/mobile/phone/notify/notifyInfo.jsp?sid="+sid+"&isLooked="+readType+"&typeId=?typeId=<%=typeId%>");
			}else{
				window.location = "notifyInfo.jsp?sid="+sid+"&isLooked="+readType+"&typeId=?typeId=<%=typeId%>";
			}
