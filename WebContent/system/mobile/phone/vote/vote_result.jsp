	...
	if (jsonObj.rtState) {
		...
		if (prc && prc.sid) {
			...
			if(attaches.length>0){
				 ...
				 $("#attachment").show();
				 $.each(attaches, function(index, item){  
					  $("#attachment").append("<div ><a href='javascript:void(0);'  onclick=\"GetFile('"+item.sid+"','"+item.fileName+"','"+item.attachmentName+"')\" >"+item.fileName + "&nbsp;&nbsp;"+"</a><div>");
	...
	for(var i=0;i<subjects.length;i++){
		...
		if(subjectItem.type=='2' || subjectItem.type=='3'){//单行文本框或多行文本框
			...
			for(var j=0;j<voteItems.length;j++){
				...
				html.push("<td nowrap style='border-bottom:1px solid #ccc;'>");
				var hasExist = false;
				for(var k=0;k<myitems.length;k++){
				...
				html.push("</span>");
				html.push("</td>");
				html.push("<td nowrap style='border-bottom:1px solid #ccc;'  class='progress0' percent='"+voteItems[j].total/total+"'></td>");
				html.push("<td nowrap style='border-bottom:1px solid #ccc;'>"+voteItems[j].total+"票</td>");
				html.push("<td nowrap class='' style='border-bottom:1px solid #ccc;'><span title='"+voteItems[j].voteUserName+"'></span></td>");
				html.push("<tr>");
			}
			html.push("</table>");
		}
