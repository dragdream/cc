...
<%
   ...
   String option=TeeStringUtil.getString(request.getParameter("option"), "wjs");//用来控制返回
%>
...
function doInit(){
	...
	b1.addEventListener("tap",function(){
		window.location='index.jsp?sid=<%=sid%>'+'&&option='+option;
	});
	
	//办理情况
	b2.addEventListener("tap",function(){
		window.location='feedBackRecords.jsp?supId=<%=sid%>'+'&&option='+option;
	});
	
	//催办记录
	b3.addEventListener("tap",function(){
		window.location='urgeRecords.jsp?supId=<%=sid%>'+'&&option='+option;
	});
	
	//办结申请记录
	b4.addEventListener("tap",function(){
		window.location='endApplyRecords.jsp?supId=<%=sid%>'+'&&option='+option;
	});
	//暂停恢复申请记录
	b5.addEventListener("tap",function(){
		window.location='pauseRecoverApplyRecords.jsp?supId=<%=sid%>'+'&&option='+option;
	});
}
...
//创建子任务
function create(){
	var url="addOrUpdate.jsp?parentId="+sid+"&&option="+option;
	window.location=url;
}
...
//任务催办
function urge(){
	var url="doUrge.jsp?supId="+sid+"&&option="+option;
	window.location=url;
...
//发表反馈
function feedback(){
	var url="doFeedBack.jsp?supId="+sid+"&&option="+option;
	window.location=url;
}
...
//申请暂停   //申请恢复    //申请办结
function apply(type){
	var url="apply.jsp?supId="+sid+"&&type="+type+"&&option="+option;
	window.location=url;
}
</script>
<body onload="doInit()">
