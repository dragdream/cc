...
function sync(){
	if(window.confirm("是否同步操作到微信？")){
		bsWindow(contextPath+"/weixinDept/syncOrg.action","同步详情",{});
	}
}
...
function syncDept(){
	...
	if(nodes.length>0){
		...
		if(deptId.indexOf("dept")==-1){//组织机构名
			...
			bsWindow(contextPath+"/weixinDept/syncDept.action?deptId="+dId,"部门同步详情",{});	
		}	
	}else{
		alert("请选择部门！");
	    return  false;
...
function  delDingDept(){
	...
	if(nodes.length>0){
		...
		if(deptId.indexOf("dept")==-1){//组织机构名
			...
			var dId=deptId.split(";")[0];
			bsWindow(contextPath+"/weixinDept/delWeiXinDept.action?deptId="+dId,"删除部门详情",{});	
		}	
	}else{
		alert("请选择部门！");
	    return  false;
...
<body onload="doInit();">
	<fieldset>
		...
		<span class="help-block">注：将OA系统中的组织机构同步到微信企业号后，微信企业号的所有账户全部归属到顶级部门，请重新进行用户同步。</span>
