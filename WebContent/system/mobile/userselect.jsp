...
<div class="mui-input-group" style="position:fixed;top:40px;left:0px;right:0px;height:40px;">
	<div class="mui-input-row">
		<input id="searchBox" type="text" placeholder="请输入人员姓名或账号搜索"  oninput="doSearch()" />
	</div>
</div>
<div class="mui-content" style="margin:0px;padding:0px;position:fixed;top:95px;left:0px;right:130px;bottom:0px;overflow:auto">
	<ul class="mui-table-view" id="deptList" style="margin:0px;padding:0px;">
			
	</ul>
</div>
<div class="mui-content" style="margin:0px;padding:0px;position:fixed;top:95px;right:0px;bottom:0px;width:128px;overflow:auto">
	<ul class="mui-table-view" id="userList" style="margin:0px;padding:0px;">
...
function renderDept(level,node){
	...
	//渲染node
	var render = [];
	render.push("<li class=\"mui-table-view-cell mui-media\" id=\"node"+node.id+"\" onclick=\"showRightUser("+node.id+")\" >");
...
function showRightUser(deptId){
	$("#userList").html("");
	var json = tools.requestJsonRs(contextPath+"/orgSelectManager/getSelectUserByDept.action",{deptId:deptId});
	var rows = json.rtData;
	for(var i=0;i<rows.length;i++){
		renderItem({name:rows[i].userName,id:rows[i].uuid});
	}
}
...
function renderItem(item){
	...
	var render = [];
	render.push("<li uuid="+item.id+" class='item mui-table-view-cell "+(exists?"selected":"")+"' onclick='clickIt(this)'>");
...
function clickIt(obj){
	...
	if($(obj).attr("class").indexOf("selected")!=-1){//如果是已被选中的，则取消选中
		...
		if(call_back_name!="undefined" && call_back_name!="null"){
			eval("parent."+call_back_name+"(\""+$(obj).attr("uuid")+"\",\""+$(obj).text()+"\",\"REMOVE_ITEM\")");
		}
	}else{//如果未被选中，则开启选中
		if(single_select){
			cancelAll();
		...
		if(call_back_name!="undefined" && call_back_name!="null"){
			eval("parent."+call_back_name+"(\""+$(obj).attr("uuid")+"\",\""+$(obj).text()+"\",\"ADD_ITEM\")");
		}
	}
...
function doSearch(){
	...
	if(hitCount==0){
		$("#userList").html("<br/><center style='font-size:12px'>请选择人员</center>");
	}
}
