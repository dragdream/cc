 ...
 */
 ...
 function getPersonTree(){
		...
		if(jsonObj.rtState){
			...
			if(json.sid){
				var url = "<%=contextPath %>/dingding/getDeptInfos.action";
				var config = {
					zTreeId:"orgZtree",
					requestURL:url,
					param:{"para1":"111"},
...
var dingIdG;
 ...
 function refresh(){
	 var json = tools.requestJsonRs(contextPath+"/dingding/getPersonsByDept.action?id="+nodeId);
	 	var render = [];
	 	for(var i=0;i<json.rtData.length;i++){
	 		render.push("<tr>");
	 		render.push("<td class='TableData'>"+json.rtData[i].name+"</td>");
	 		render.push("<td class='TableData'>"+json.rtData[i].userid+"</td>");
	 		if(json.rtData[i].active){
	 			render.push("<td class='TableData' style='color:green'>已绑定("+json.rtData[i].position+")</td>");
	 		}else{
	 			render.push("<td class='TableData' style='color:gray'>未绑定</td>");
	 		}
	 		if(json.rtData[i].active){
	 			render.push("<td class='TableData'><button class='btn btn-warning' onclick=\"calcelBind('"+json.rtData[i].userid+"','"+json.rtData[i].avatar+"')\">解绑</button></td>");
	 		}else{
	 			render.push("<td class='TableData'><button class='btn btn-success' onclick=\"bind('"+json.rtData[i].userid+"','"+json.rtData[i].avatar+"')\">绑定</button></td>");
	 		}
	 		render.push("</tr>");
	 	}
 ...
 function calcelBind(dingId,userId){
	 if(window.confirm("确认要解绑该账号吗？")){
		 var json = tools.requestJsonRs(contextPath+"/dingding/bindUser.action?type=cancel",{dingId:dingId,userId:userId});
		 refresh();
	 }
 ...
 function ORG_SELECT_ADD_BACH_FUNC(a,b,c,d,e,f,g){
	 ...
	 if(window.confirm("确认要绑定["+d+"]到钉钉账号吗？")){
		 var json = tools.requestJsonRs(contextPath+"/dingding/bindUser.action?type=bind",{dingId:dingIdG,userId:c});
		 refresh();
	 }
 }
