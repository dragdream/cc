	...
	<body>
		...
		<script type="text/javascript">
			...
			function renderAllPeople(value){
				...
				$.ajax({
					url: "/orgSelectManager/getSelectUserByUserIdOrUserNameWorkflow.action",
					type: "post",
					data:{
						user : keyWord,frpSid:frpSid,prcsId:prcsId
					},
					dataType:"json",
			 ...
			 * 同部门
			 */
			function renderSameDept(personData){
				
				var loadingBox = $('<div class="mui-loading"><div class="mui-spinner"></div></div>');
				$(".sameDeptWrap").find(".mui-input-group").html(loadingBox);
				var rtData = personData;
			...
			*/
			...
			function renderFrequentGroupContent (groupId) {
				...
				$.ajax({
					url: "/orgSelectManager/getPersonByUserGroupWorkFlow.action",
					type: "post",
					data:{
						userGroupId : groupId,frpSid:frpSid,prcsId:prcsId
					},
					dataType:"json",
					success:function(data, textStatus){
						if(data.rtState){
			...
			function getOrgDepAll(){
				...
				$.ajax({
					url: "/orgSelectManager/getSelectDeptTreeWorkFlow.action",
					type: "post",
					data:{
						frpSid:frpSid,prcsId:prcsId
					},
					dataType:"json",
					success:function(data, textStatus){
						if(data.rtState){
							var rtData = data.rtData.ztreeData;
							var treeData = toTreeData(rtData,"id","pId","child");
							var html = renderOrgDep(treeData)
							$(".organizationWrap").html(html);
							renderSameDept(data.rtData.personData);
						}
					},
					error:function(XMLHttpRequest, textStatus, errorThrown){
						if(textStatus){
			...
			function renderOrgDepPerson(deptId){
				...
				$.ajax({
					url: "/orgSelectManager/getSelectUserByDeptWorkFlow.action",
					type: "post",
					data:{
						deptId: deptId,frpSid:frpSid,prcsId:prcsId
					},
					dataType:"json",
					success:function(data, textStatus){
						if(data.rtState){
