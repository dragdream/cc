        ...
        var pagination = {
            ...
            initialize: function(){
                ...
                self.parseDataSource(attributes.dataSource, function(dataSource){
                    ...
                    postData['rows'] = attributes.pageSize;
                    postData['page'] = 1;
                    ...
                    formatAjaxParams.success = function(response){
                    	attributes.totalNumber = response.total;
                    	// Obtain the total number of pages
                    	self.model.totalPage = self.getTotalPage();
                    	
                    	attributes.callback(response);
                    	
                    	// Less than one page
                        if(attributes.hideWhenLessThanOnePage){
                            if(self.model.totalPage <= 1) return;
                        }
            ...
            go: function(number, callback){
            	if(attributes.goPage){
            		attributes.goPage(number,attributes);
            	}
                var self = this;
                var model = self.model;
