(function($){
	...
	function renderTab(c1,c2,opts){
		...
		if(!outerTabContainer){
			...
			var center = $("<div id='nav_width_a' style='overflow:hidden'></div>");
			
			c1.append(left).append(right).append(center);
			c1[0].tab_center = center[0];
			
			outerTabContainer = $("<div id='nav_width' style='width:100000000px;'></div>");
		...
		var closeBtn = $("<i class='tee_close'></i>").hide();
		tab.append(closeBtn);
		...
		closeBtn.click(function(){
			...
			tab.remove();//el.src = 'about:blank';
			tabContent.attr('src', 'about:blank');  
