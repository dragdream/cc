...
getAttachElement:function(attachModel,opts){
	...
	if((priv & 1)==1){//阅读权限
		if(this.isImage(ext)){//如果是图片
			menuData.push({name:"预览",action:function(sid){
				//var url = contextPath +"/oa/core/attachment/picExplore.jsp?id="+sid;
			...
			menuData.push({name:"查看",action:function(sid,fileName){
				var ntkoURL = contextPath +  "/oa/core/ntko/indexNtko.jsp?attachmentId="+sid+"&attachmentName="+fileName+"&moudle=workFlow&op=7";
	...
	if((priv & 8)==8 && this.isOffice(ext)){//编辑权限
		menuData.push({name:"编辑",action:function(sid,fileName){
			var ntkoURL = contextPath +  "/oa/core/ntko/indexNtko.jsp?attachmentId="+sid+"&attachmentName="+fileName+"&moudle=workFlow&op=4";
	...
	if((priv & 2)==2){//转储权限
		menuData.push({name:"转存",action:function(sid){
			openWindow(contextPath+"/oa/core/base/saveFileTo/index.jsp?attachId="+sid,"附件转存",480,350);
		} ,extData:[attachModel.sid]});
	}
