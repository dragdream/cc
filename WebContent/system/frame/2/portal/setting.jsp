...
function getChildMenu(parentMenuId , parentID){
	...
	$.each(notSelectSysMenuModelList, function(j, m) {
        ...
        if(!parent.$("#block_"+m.uuid).length > 0 && parentMenuId == menuId && m.menuId.length != 3 ){//不是父节点，且未在桌面的
	         ...
	         var a = $('<a title="'+m.menuName+'" href="javascript:void(0);" id="'+m.uuid+'"></a>').append('<img width="48" height="48"  src="' + cssPathSecond + '/images/app_icons/' +icon+'"><span class="lleft"><span class="lright">'+m.menuName+'</span></span>');
...
<div >
	<button type="button" class="btn btn-primary"  onclick="setMemu()">应用模块设置</button> &nbsp;&nbsp;&nbsp;&nbsp;
	<button type="button" class="btn btn-primary" onclick="setScreen()">分屏设置</button>
</div>

<div id="app_menu_setting" style="margin-top:10px;">
	    <div class="ui-layout-west" id="app_cate_list">
			<div class="list-group"  id="menuParent" style="height:310px ;overflow-y:auto;"></div>
		</div>
