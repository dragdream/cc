...
function getChildMenu(parentMenuId , parentID){
	...
	$.each(notSelectSysMenuModelList, function(j, m) {
        ...
        if(!parent.$("#block_"+m.uuid).length > 0 && parentMenuId == menuId && m.menuId.length != 3 ){//不是父节点，且未在桌面的
	         ...
	         var a = $('<a title="'+m.menuName+'" href="javascript:void(0);" id="'+m.uuid+'"></a>').append('<img width="48" height="48"  src="' + contextPath + '/system/frame/3/icons/' +icon+'"><span class="lleft"><span class="lright">'+m.menuName+'</span></span>');
	         if(m.menuCode!=""){
