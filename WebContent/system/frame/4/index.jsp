...
<link rel="stylesheet" type="text/css" href="<%=contextPath%>/system/frame/3/style/css/css.css">
<link rel="stylesheet" type="text/css" href="<%=contextPath%>/system/frame/3/style/css/general.css">
<script type="text/javascript" src="<%=contextPath%>/system/frame/3/js/jquery.SuperSlide.js?v=2"></script>
<script type="text/javascript" src="<%=contextPath%>/system/frame/4/js/index2.js?v=15"></script>
...
<div id="header">
	...
	if(!"".equals(TOP_BANNER_TEXT)){//有标题，则优先显示标题
		%>
		<table style="height:56px;border-collapse:collapse;width:100%">
			<tr>
				<td style="font-size:14px;color:white;font-family:微软雅黑;font-weiht:bold;line-height:16px;padding-left:10px;">
					<%=TOP_BANNER_TEXT %>
				</td>
			</tr>
		</table>
		...
		%>
		<img src="/attachmentController/downFile.action?id=<%=TOP_ATTACHMENT_ID %>"  style="height:40px;margin-top:8px" />
		<%
	}else{
		%>
		<img src="logo.png" style="height:40px;margin-top:8px"/>
		<%
	}
	%>
		</div>
        ...
        <div class="header_right">
                ...
                <div class="weibo" title="企业社区" onclick="addTabFunc('企业社区','/system/core/base/weibo/index.jsp');"></div>
                <div class="sms">
...
<div id="footer" style="overflow:hidden">
	<span style='display:inline-block;float:left;margin-left:15px;font-size:12px'>当前登录人：<font><%=userName %></font></span>
	<%
...
<div class="smsbox" id="smsbox" style="display:none">
	...
	<span style="float:right;cursor:pointer" onclick="$('#smsbox').hide();$('.tip_zhezhao').hide()">×</span>
	<span style="float:right;cursor:pointer" class="smsbox_icon2" onclick="$('.tip_zhezhao').hide();smsDetails();">消息列表</span>
	<span style="float:right;cursor:pointer" class="smsbox_icon1" onclick="$('.tip_zhezhao').hide();smsViewAlls();">全部已阅</span>
