        ...
        // 无新短信提示
          ...
          $('#smsbox_detail_all').click(
              function() {
                ...
                var url = contextPath + '/system/frame/ispirit/nav.jsp?smsIds=' + sms_id_str;
                openURL(url);
                selectedRecvSmsIdStr = selectedSendSmsIdStr = '';
              });
              ...
              function() {
                ...
                for ( var i = 0; i < newSmsArray.length; i++) {
                  ...
                  if (selectedRecvSmsIdStr.indexOf(id + ',') == 0
                      || selectedRecvSmsIdStr.indexOf(',' + id + ',') > 0) {
                    nav_mail_url = newSmsArray[i].url;
                    if (newSmsArray[i].type_id == '0')
                      sms_id_str0 += id + ',';
                    continue;
                ...
                markRead(selectedRecvSmsIdStr);
                var url = contextPath + '/system/frame/ispirit/nav.jsp?smsIds=' + selectedRecvSmsIdStr;
                openURL(url);
                newSmsArray = array;
                FormatSms();
              });
              ...
              .click(
                  function() {
                    ...
                    var html = CreateMsgBlock( {
                      ...
                      "time" : date.toTimeString().substr(0, 11),
                      "content" : content
                    });
                    ...
                    newSmsArray[newSmsArray.length] = {
                      ...
                      type_id : "0",
                      type_name : "个人短信",
                      send_time : date.toTimeString().substr(0, 11),
                      unread : 0,
                      content : content,
                      url : "",
                      receive : 0
                    ...
                    jQuery.ajax( {
                      type : 'POST',
                      url : contextPath + '/sms/addSms.action',
                      data : {
                        'userListIds' : user_id,
                        'content' : msg,
                        'smsType' : '0'
...
function RemoveSms(recvIdStr, sendIdStr, del) {
  ...
  if(del == 0){
    ...
    url = contextPath + "/sms/delSms.action?";
    data = {
        'bodyId' : recvIdStr,
        'deType':1,
        'DEL' : del
...
function openURL(url) {
  ...
  var myleft = (screen.availWidth - 840) / 2;
  window
      .open(
          url,
          "查看详情",
          "height=548,width=900,status=0,toolbar=no,menubar=yes,location=no,scrollbars=yes,top="
...
function send_sms(seqId, fromId) {
  ...
  window
      .open(
          contextPath + "/system/core/sms/addSms.jsp?fromId="
              + fromId,
          "",
          "height=548,width=900,status=0,toolbar=no,menubar=no,location=no,scrollbars=no,top="
              + mytop + ",left=" + myleft + ",resizable=yes");
}
...
if (top.Tee && top.Tee.isTouchDevice) {
  window.showModalDialog = function () {
    top.Tee && top.showModalDialog.apply(this, arguments);
  };
  
  window.open = function () {
    top.Tee && top.open.apply(this, arguments);
  };
}
