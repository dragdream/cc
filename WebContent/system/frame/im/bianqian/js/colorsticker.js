	...
	Sticker.prototype = {
			...
			if(hasMovingSticker){
				...
				for(var i=0;i<stickers.length;i++){
					if(stickers[i].moving){
						...
						var json=tools.requestJsonRs("/noteManage/updateLoc.action",{sid:stickers[i].getAttribute("stickerId"),left:stickers[i].style.left,top:stickers[i].style.top});
		...
		closeSticker : function(event){
			...
			if($.isFunction(Sticker.options.closeStickerCallback)) {
				isClose = Sticker.options.closeStickerCallback.call(this, this.parentElement.getAttribute('stickerId'));
			}
		...
		saveSticker : function(event){
			...
			if($.isFunction(Sticker.options.saveStickerCallback)) {
							var stickerItem = {
								...
								stickerId:stickerEl.getAttribute('stickerId')
							} 
