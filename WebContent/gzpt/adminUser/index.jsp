...
function doInit(){
	$("#group").group();
	changePage('<%=contextPath%>/gzpt/adminUser/query.jsp');
	getPersonTree();
...
function personOnClick(event, treeId, treeNode) {
	...
	if(uuid.split(";").length == 2 && uuid.split(";")[1] == 'dept'){
		changePage("<%=contextPath%>/gzpt/adminUser/personList.jsp?deptId=" + uuid.split(";")[0] + "&deptName=" + encodeURIComponent(treeNode.name));
	}else if(uuid.split(";").length == 2 && uuid.split(";")[1] == 'personId'){
		//parent.personinput.location = "<%=contextPath%>/gzpt/adminUser/personList.jsp?uuid=" + uuid.split(";")[0] ;
		
		//var url = contextPath +  "/system/core/person/addupdate.jsp?uuid=" + personUuid ;
		changePage(contextPath + "/system/core/person/addupdate.jsp?uuid="+uuid.split(";")[0]);
	}else if(uuid.split(";").length == 1){//点击顶层加点
		changePage("<%=contextPath%>/gzpt/adminUser/personList.jsp?deptId=0&orgId="+uuid.split(";")[0]+"&deptName=" + encodeURIComponent(treeNode.name));
...
<div class="panel-group" id="list-group">
  ...
  <div class="panel panel-default">
    <div class="panel-heading menuList dom">
      <h4 class="panel-title">
        <a data-toggle="collapse" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" onclick="toQuery();">
         	用户查询或导出
        </a>
      </h4>
    </div>
  ...
  <div class="panel panel-default">
   ...
   <div class="panel panel-default">
    <div class="panel-heading menuList dom">
      <h4 class="panel-title">
        <a data-toggle="collapse" data-toggle="collapse" data-parent="#accordion111" href="#" onclick="toMultiSet();">
       		  用户批量设置
        </a>
      </h4>
    </div>
