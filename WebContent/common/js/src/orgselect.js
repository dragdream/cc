...
function selectUserWorkFlow(retArray , moduleId,privNoFlag ,filter,callBackPara) {
  ...
  if(filter){
	  window.PRCSID = filter.split("_")[0];
	  window.FRPSID = filter.split("_")[1];
  }
...
function selectSingleRole(retArray, moduleId,privNoFlag , privOp) {
	  roleRetNameArray = retArray;
	  objSelectType = retArray[2] || "";
	  var url =contextPath + "/system/core/orgselect/selectMultiRole2.jsp?objSelectType=" + objSelectType + "&isSingle=1";
	  if (moduleId) {
	    url += "&moduleId=" + moduleId;
	    if (!privNoFlag) {
	      privNoFlag = 0;
