...
function listDatagrid(){
	datagrid = $('#datagrid') .datagrid( {
		url : contextPath + '/SuperviseController/findListBypage.action',
		...
		columns : [ [
				...
				{ field : 'name', title : '机关全称',align:'left' ,width:'18%', halign: 'center', formatter: 
					function(e, rowData) {
 ...
 */
function edit(id) {
	top.bsWindow(contextPath + "/supervise/supervise/supervise_add.jsp?id="+id, "修改",
			{
				width : "780",
				height : "300",
				buttons : [ {
	...
	if(checkedItems.length == 1){
		...
		if(userName =="" || userName == null){
			...
			top.$.MsgBox.Confirm("提示","重置后密码为：zfjd123456，确定重置？",function(){
				var json = tools.requestJsonRs("/SuperviseController/resetPassword.action",{id:checkedItems[0].id});
				$('#datagrid').datagrid("reload");
				$('#datagrid').datagrid("clearSelections");
	...
	if(checkedItems.length == 1){
		...
		if(userName =="" || userName == null){
			...
			top.$.MsgBox.Confirm("提示","确定回收该账号？",function(){
				var json = tools.requestJsonRs("/SuperviseController/resetUser.action",{id:checkedItems[0].id});
				$('#datagrid').datagrid("reload");
				$('#datagrid').datagrid("clearSelections");
			});
		}
	...
	if(checkedItems.length == 1){
		...
		if(examine == 1){
			...
			if(userName =="" || userName == null){
				...
				top.bsWindow(contextPath + "/supervise/supervise/supervise_login.jsp?id="+id, "分配账号", {
					width : "420",
					height : "130",
					buttons : [ {
						name : "关闭",
