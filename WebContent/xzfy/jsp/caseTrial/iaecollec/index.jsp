...
<body style="padding-left: 10px;padding-right: 10px" onLoad="doInit();";>
   ...
   <%-- <div id="toolbar" class = " clearfix" style="margin-top: 5px">
   	...
   	<div class='tab-container' id="container">
	   	<span id="needDeal" class='actived-tables' onclick="change(this,'0')">调查取证</span>
	   	<span id="alreadyDeal" class='case-tab' onclick="change(this,'1')">听证管理</span>
	   	<span id="lastDeal" class='case-tab' onclick="change(this,'2')">集体讨论会</span>
   	</div>
    
    <iframe id="exportIframe" style="width:100%;height:90%" src=""></iframe>
    ...
    function doInit(){
		...
		function change(that,type){
			...
			//给iframe更换src
			if(type==0){
				$("#exportIframe").attr("src","/xzfy/jsp/caseTrial/investigation/index.jsp?caseId="+caseId);
			}
			if(type==1){
				$("#exportIframe").attr("src","/xzfy/jsp/caseTrial/caseHearing/index.jsp?caseId="+caseId);
			}
			if(type==2){
				$("#exportIframe").attr("src","/xzfy/jsp/caseTrial/caseDiscussion/index.jsp?caseId="+caseId);
			}
			$("#exportIframe").show();
		}
     </script>
