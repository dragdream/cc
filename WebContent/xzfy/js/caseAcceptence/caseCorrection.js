...
function chooseReason(){
	bsWindow("/xzfy/jsp/caseAcceptence/chooseCorReason.jsp", "选择承办人", {
		width : "750",
		height : "250",
		buttons:
		[
...
//选择补正材料
function chooseMatial(){
	bsWindow("/xzfy/jsp/caseAcceptence/chooseCorMatiral.jsp", "选择承办人", {
		width : "750",
		height : "250",
		buttons:
		[
		...
		submit : function(v, h, c, b) {
			var result = h[0].contentWindow;
			if(v == "保存"){
				var reason = result.giveFatherMatial();
				//将中止理由回显
				$("#remark").val(reason);
				//关闭子页面
				return true; 
